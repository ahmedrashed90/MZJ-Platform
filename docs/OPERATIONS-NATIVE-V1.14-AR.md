...
## تصحيح v1.14.1
تم توحيد ملف الـMigration الخارجي مع الـMigration المضمّنة داخل `server/_operations-schema.ts`، وإضافة أعمدة التوافق لجدول `operations.vehicle_statuses` قبل إدخال الحالات. يمنع ذلك ظهور رسالة «تعذر تنفيذ العملية حاليًا» عند أول تحميل لو كانت قاعدة البيانات تحتوي على النسخة القديمة من الجدول.

## تحديث v1.14.2
- إزالة تبويب «إعدادات العمليات» من شريط التنقل الداخلي لنظام العمليات، مع بقائها داخل صفحة إعدادات المنصة الموحدة.
- توسيط فلاتر طلبات النقل والتصوير في منتصف الصفحة مع السماح بالتفافها على الشاشات الضيقة.
- تعديل قالب استيراد السيارات ليستخدم «المكان» و«الحالة» بالعربية بدل مطالبة المستخدم بالأكواد الداخلية.
- قبول شيت النظام القديم مباشرة، بما يشمل أعمدة «الدفعة»، «ملاحظة المكان»، «ملاحظة النواقص»، وتجاهل عمود Tracking.
- قبول الأسماء العربية والأكواد الداخلية في الاستيراد، وتحويل «حجز» و«محجوز» تلقائيًا إلى الحالة الداخلية الصحيحة.
- تصدير بيانات السيارات بالأسماء العربية للمكان والحالة دون إظهار الأكواد الداخلية للمستخدم.
