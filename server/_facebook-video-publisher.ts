...
function maybeNestedJson(value: unknown) {
...
function facebookErrorMessage(payload: any, status: number, fallback: string) {
  ...
  const nested = maybeNestedJson(error?.message || payload?.message);
  const nestedError = nested?.error || nested || {};
  const userTitle = clean(error?.error_user_title || nestedError?.error_user_title);
  const userMessage = clean(error?.error_user_msg || nestedError?.error_user_msg);
  const primary = clean(
    userMessage
    || error?.message
    || nestedError?.message
...
async function openVideoUploadSource(sql: Sql, file: any): Promise<VideoUploadSource> {
  ...
  const headerLength = positiveInteger(response.headers.get("content-length"));
  const storedLength = positiveInteger(file?.file_size);
  const contentLength = headerLength || storedLength;
