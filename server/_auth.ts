...
export function isSystemAdmin(user: Pick<SessionUser, "roleCodes">) {
  return user.roleCodes.some((code) => code === "admin" || code === "system_admin");
}

export function hasPermission(user: Pick<SessionUser, "roleCodes" | "permissionCodes">, permission: string) {
...
export async function requirePermission(request: VercelRequest, response: VercelResponse, permission: string) {
  ...
  if (!hasPermission(user, permission)) {
    response.status(403).json({ ok: false, code: "FORBIDDEN", error: "ليس لديك صلاحية تنفيذ هذا الإجراء", message: "ليس لديك صلاحية تنفيذ هذا الإجراء" });
    return null;
  }
  return user;
}
