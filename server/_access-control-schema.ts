...
('settings.users.delete','حذف مستخدم','core','settings','users_delete','حذف مستخدم','حذف الحساب وإزالة بيانات دخوله مع الاحتفاظ بالسجلات السابقة','settings',true,105,true),
...
select r.id,p.id from core.roles r join core.permissions p on p.code='settings.users.delete'
...
const ACCESS_CONTROL_SCHEMA_VERSION = 1195;
...
async function accessControlSchemaReady() {
  ...
  const [shape] = await sql<{ ready: boolean }[]>`
    select
      ...
      and exists(select 1 from information_schema.columns where table_schema='core' and table_name='users' and column_name='deleted_at')
      and exists(select 1 from information_schema.columns where table_schema='core' and table_name='users' and column_name='deleted_by')
      and exists(select 1 from information_schema.columns where table_schema='core' and table_name='users' and column_name='deleted_reason')
