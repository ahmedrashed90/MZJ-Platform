...
-- Consolidate the legacy duplicate marketing role into the canonical template.
-- Existing users and permissions are preserved, then the duplicate is hidden.
insert into core.role_permissions(role_id,permission_id)
select canonical.id,rp.permission_id
from core.roles legacy
join core.roles canonical on canonical.code='marketing_user'
join core.role_permissions rp on rp.role_id=legacy.id
where legacy.code='marketing_executive'
on conflict(role_id,permission_id) do nothing;

update core.users u
set permission_version=permission_version+1,updated_at=now()
where exists (
  select 1 from core.user_roles ur join core.roles legacy on legacy.id=ur.role_id
  where ur.user_id=u.id and legacy.code='marketing_executive'
) or exists (
  select 1 from core.user_systems us join core.roles legacy on legacy.id=us.role_id
  where us.user_id=u.id and legacy.code='marketing_executive'
);

delete from core.sessions s
where exists (
  select 1 from core.user_roles ur join core.roles legacy on legacy.id=ur.role_id
  where ur.user_id=s.user_id and legacy.code='marketing_executive'
) or exists (
  select 1 from core.user_systems us join core.roles legacy on legacy.id=us.role_id
  where us.user_id=s.user_id and legacy.code='marketing_executive'
);

insert into core.user_roles(user_id,role_id)
select ur.user_id,canonical.id
from core.user_roles ur
join core.roles legacy on legacy.id=ur.role_id and legacy.code='marketing_executive'
join core.roles canonical on canonical.code='marketing_user'
on conflict(user_id,role_id) do nothing;

update core.user_systems us
set role_id=canonical.id,updated_at=now()
from core.roles legacy,core.roles canonical
where us.role_id=legacy.id
  and legacy.code='marketing_executive'
  and canonical.code='marketing_user';

delete from core.user_roles ur
using core.roles legacy
where ur.role_id=legacy.id and legacy.code='marketing_executive';
