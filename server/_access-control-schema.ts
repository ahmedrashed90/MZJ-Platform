...
insert into core.role_permissions(role_id,permission_id)
select r.id::uuid,p.id::uuid from core.roles r join core.permissions p on p.code in ('operations.approval.financial','operations.approvals.view','operations.inventory.view','operations.vehicle.view','system.operations.access') where r.code='finance_manager'
on conflict do nothing;
...
select u.id,s.code,true,
  case when s.code='crm' then (select r.id::uuid from core.roles r join core.user_roles ur on ur.role_id=r.id::uuid where ur.user_id=u.id and r.code in ('sales_manager','branch_manager','sales_user','call_center_agent','customer_service_agent') order by case r.code when 'sales_manager' then 1 when 'branch_manager' then 2 else 3 end limit 1)
       when s.code='marketing' then (select r.id::uuid from core.roles r join core.user_roles ur on ur.role_id=r.id::uuid where ur.user_id=u.id and r.code in ('marketing_admin','marketing_user') order by case r.code when 'marketing_admin' then 1 else 2 end limit 1)
       when s.code='operations' then (select r.id::uuid from core.roles r join core.user_roles ur on ur.role_id=r.id::uuid where ur.user_id=u.id and r.code in ('operations_manager','operations_admin','operations_user','finance_manager','accounts_manager') order by case r.code when 'operations_manager' then 1 when 'operations_admin' then 2 else 3 end limit 1)
       when s.code='tracking' then (select r.id::uuid from core.roles r join core.user_roles ur on ur.role_id=r.id::uuid where ur.user_id=u.id and r.code='tracking_user' limit 1)
  end,
  case
...
insert into core.role_permissions(role_id,permission_id)
select r.id::uuid,p.id::uuid from core.roles r join core.permissions p on p.code in ('crm.data_review.view','crm.data_review.execute')
