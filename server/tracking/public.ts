...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  return response.status(200).json({
    ...
    order: {
      ...
      package_group: order.package_group,
      package_price_before_tax: order.package_price_before_tax,
      package_tax_value: order.package_tax_value,
      package_total_incl_vat: order.package_total_incl_vat,
      package_cash_discount_percent: order.package_cash_discount_percent,
      status: order.status,
