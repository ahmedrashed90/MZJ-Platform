...
async function loadConversationAutomationSettings(sql: any) {
  await sql`
    insert into crm.conversation_automation_platforms(platform_code,worker_code,is_enabled,connection_status,health_url,metadata)
    select lower(e.source_code),e.source_code,false,
      'unknown',
      e.health_url,jsonb_build_object('discoveredFromIntegrationEndpoint',true)
  ...
  const [settings, startMessages, platforms, workers, flows, sessions] = await Promise.all([
    sql`select * from crm.conversation_automation_settings where id='default'`,
    sql`select *,id::text from crm.conversation_automation_start_messages order by sort_order,id`,
    sql`
    ...
    sql`
      select f.*,f.id::text,
        ...
        ) order by a.created_at) from crm.conversation_automation_flow_aliases a where a.flow_id=f.id),'[]'::json) as aliases,
        coalesce((select json_agg(json_build_object(
        ...
        ) order by s.sort_order,s.id) from crm.conversation_automation_flow_steps s where s.flow_id=f.id),'[]'::json) as steps
    ...
    sql`
      ...
      left join crm.conversation_automation_final_actions fa on fa.session_id=s.id
...
async function handleRequest(request: VercelRequest, response: VercelResponse) {
  ...
  if (!schema.ready) {
    return response.status(409).json({
      ok: false,
      code: "CRM_CONVERSATION_AUTOMATION_MIGRATION_REQUIRED",
      error: `إعدادات الأوتوميشن غير جاهزة. نفّذ ملف ${schema.migration} مرة واحدة على PostgreSQL ثم حدّث الصفحة.`,
  ...
  if (request.method === "GET") {
    ...
    return response.status(200).json({ ok: true, ...(await loadConversationAutomationSettings(sql)) });
  }
  ...
  if (request.method === "DELETE") {
    ...
    if (section === "start_message") {
      await sql`delete from crm.conversation_automation_start_messages where id=${id}::uuid`;
      await audit(user, "crm_conversation_automation_start_message_deleted", "conversation_automation_start_message", id);
      return response.status(200).json({ ok: true, message: "تم حذف رسالة البداية" });
    }
    if (section === "flow") {
      const [usage] = await sql<any[]>`
        select exists(select 1 from crm.conversation_automation_sessions where flow_id=${id}::uuid) as used
      `;
      if (usage?.used) {
        await sql`update crm.conversation_automation_flows set is_active=false,updated_at=now() where id=${id}::uuid`;
        await audit(user, "crm_conversation_automation_flow_deactivated", "conversation_automation_flow", id);
        return response.status(200).json({ ok: true, deactivated: true, message: "تم إيقاف الاختيار لأنه مستخدم في جلسات سابقة" });
      }
      await sql`delete from crm.conversation_automation_flows where id=${id}::uuid`;
      await audit(user, "crm_conversation_automation_flow_deleted", "conversation_automation_flow", id);
      return response.status(200).json({ ok: true, message: "تم حذف الاختيار" });
    }
    if (section === "step") {
      const [usage] = await sql<any[]>`
        select exists(select 1 from crm.conversation_automation_answers where step_id=${id}::uuid)
          or exists(select 1 from crm.conversation_automation_sessions where current_step_id=${id}::uuid) as used
      `;
      if (usage?.used) {
        await sql`update crm.conversation_automation_flow_steps set is_active=false,updated_at=now() where id=${id}::uuid`;
        return response.status(200).json({ ok: true, deactivated: true, message: "تم إيقاف الخطوة لأنها مستخدمة في جلسات سابقة" });
      }
      await sql`delete from crm.conversation_automation_flow_steps where id=${id}::uuid`;
  ...
  if (section === "general") {
    ...
    const [row] = await sql<any[]>`
      update crm.conversation_automation_settings set
        automation_name=${clean(body.automationName) || "أوتوميشن استقبال العملاء"},
        automation_enabled=${body.automationEnabled !== false},
        trigger_policy=${triggerPolicy},custom_interval_value=${intervalValue},custom_interval_unit=${intervalUnit},updated_by=${user.id}::uuid,updated_at=now()
      where id='default' returning *
    `;
    await audit(user, "crm_conversation_automation_general_saved", "conversation_automation_settings", "default", row);
  ...
  if (section === "start_message") {
    ...
    const [row] = id ? await sql<any[]>`
      update crm.conversation_automation_start_messages set message_key=${messageKey},message_text=${messageText},
    ...
    ` : await sql<any[]>`
      ...
      returning *,id::text
    `;
    await audit(user, "crm_conversation_automation_start_message_saved", "conversation_automation_start_message", row?.id || id, row);
  ...
  if (section === "platform") {
    ...
    const requestedWorkerCode = clean(body.workerCode);
    if (!platformCode) return response.status(400).json({ ok: false, error: "كود المنصة مطلوب" });
    let worker: any = null;
    if (requestedWorkerCode) {
      [worker] = await sql<any[]>`
        select * from crm.integration_endpoints where lower(source_code)=lower(${requestedWorkerCode}) limit 1
      `;
      if (!worker) return response.status(400).json({ ok: false, error: "الـWorker غير مسجل في إعدادات ربط المنصات" });
      if (clean(worker.source_code).toLowerCase() !== platformCode) {
        return response.status(400).json({ ok: false, error: "لا يمكن ربط Worker تابع لمنصة مختلفة" });
      }
    }
    if (body.isEnabled !== false) {
      if (!worker) return response.status(400).json({ ok: false, error: "اختر Worker للمنصة" });
    ...
    const workerCode = clean(worker?.source_code || requestedWorkerCode);
    const [row] = await sql<any[]>`
      insert into crm.conversation_automation_platforms(platform_code,worker_code,is_enabled,connection_status,health_url,updated_by,updated_at)
      values(${platformCode},${workerCode || null},${body.isEnabled !== false},${body.isEnabled !== false ? "unknown" : "disconnected"},${clean(body.healthUrl) || null},${user.id}::uuid,now())
      on conflict(platform_code) do update set worker_code=excluded.worker_code,is_enabled=excluded.is_enabled,
        connection_status=case
          when excluded.is_enabled=false then 'disconnected'
          when crm.conversation_automation_platforms.worker_code is distinct from excluded.worker_code then 'unknown'
          else crm.conversation_automation_platforms.connection_status
        end,health_url=excluded.health_url,updated_by=excluded.updated_by,updated_at=now()
      returning *,id::text
    `;
    await audit(user, "crm_conversation_automation_platform_saved", "automation_platform", row?.id || platformCode, row);
  ...
  if (section === "platform_health") {
    ...
    const [platform] = await sql<any[]>`
      ...
      from crm.conversation_automation_platforms p left join crm.integration_endpoints e on e.source_code=p.worker_code
    ...
    try {
      ...
      await sql`
        update crm.conversation_automation_platforms set connection_status=${status},
      ...
      await sql`update crm.conversation_automation_platforms set connection_status='error',last_error=${message},updated_at=now() where platform_code=${platformCode}`;
  ...
  if (section === "flow") {
    ...
    if (branchPolicy === "fixed" && !branchCode) {
      return response.status(400).json({ ok: false, error: "حدد كود الفرع عند استخدام فرع ثابت" });
    }
    ...
    const steps = list(body.steps).map((item: any, index: number) => ({
      ...
      customerField: ["customer_name","car_name","phone"].includes(clean(item.customerField)) ? clean(item.customerField) : null,
    ...
    const row = await sql.begin(async (tx) => {
      const [flow] = id ? await tx<any[]>`
        update crm.conversation_automation_flows set flow_code=${flowCode},display_name=${displayName},emoji=${clean(body.emoji) || null},
          button_payload=${clean(body.buttonPayload) || flowCode},service_key=${serviceKey},department_code=${departmentCode},
          branch_policy=${branchPolicy},branch_code=${branchPolicy === "fixed" ? branchCode : null},
          final_action=${tx.json((body.finalAction || {}) as any)},final_message=${clean(body.finalMessage)},
          is_active=${body.isActive !== false},sort_order=${Number(body.sortOrder || 10)},updated_at=now()
        where id=${id}::uuid returning *,id::text
      ` : await tx<any[]>`
        insert into crm.conversation_automation_flows(flow_code,display_name,emoji,button_payload,service_key,department_code,branch_policy,branch_code,final_action,final_message,is_active,sort_order)
        values(${flowCode},${displayName},${clean(body.emoji) || null},${clean(body.buttonPayload) || flowCode},${serviceKey},${departmentCode},
          ${branchPolicy},${branchPolicy === "fixed" ? branchCode : null},${tx.json((body.finalAction || {}) as any)},${clean(body.finalMessage)},
          ${body.isActive !== false},${Number(body.sortOrder || 10)}) returning *,id::text
      `;
      if (!flow) throw new Error("تعذر حفظ الاختيار");

      await tx`delete from crm.conversation_automation_flow_aliases where flow_id=${flow.id}::uuid`;
      ...
      for (const step of steps) {
        const [saved] = step.id ? await tx<any[]>`
          update crm.conversation_automation_flow_steps set step_key=${step.stepKey},step_name=${step.stepName},prompt_text=${step.promptText},
        ...
        ` : await tx<any[]>`
          insert into crm.conversation_automation_flow_steps(flow_id,step_key,step_name,prompt_text,step_type,customer_field,is_required,validation_rules,validation_error,max_attempts,is_active,sort_order)
      ...
      if (savedStepIds.length) {
        await tx`
          update crm.conversation_automation_flow_steps set is_active=false,updated_at=now()
          where flow_id=${flow.id}::uuid and not(id=any(${savedStepIds}::uuid[]))
        `;
      } else {
        await tx`update crm.conversation_automation_flow_steps set is_active=false,updated_at=now() where flow_id=${flow.id}::uuid`;
      }
      return flow;
    });
    await audit(user, "crm_conversation_automation_flow_saved", "conversation_automation_flow", row.id, row);
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  try {
    ...
    const message = clean(error?.message || error) || "تعذر تنفيذ إعدادات الأوتوميشن";
    console.error("CRM conversation automation settings failed", { message, method: request.method });
    if (message.includes("duplicate key") || message.includes("unique constraint")) {
      return response.status(409).json({ ok: false, error: "القيمة أو الرد المقبول مستخدم بالفعل داخل هذا الفلو." });
    }
