...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  const [statuses, branches, users, sources, quality, endpoints, templates, mappings, customerFields] = await Promise.all([
    ...
    sql`select id::text,name,display_name,content,template_type,provider,external_id,language_code,departments from crm.message_templates where is_active=true order by display_name`,
