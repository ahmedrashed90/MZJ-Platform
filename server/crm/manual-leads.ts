...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  if (request.method === "POST") {
    ...
    if (!duplicate) {
      ...
      [createdLead] = await sql<any[]>`
        ...
        values (${customerName},${phone},${phoneNormalized},${sourceCode},${sourceName},${serviceKey},${departmentCode},${branchCode},'عميل جديد',${paymentType},${clean(body.carName) || null},${clean(body.carCategory) || null},${clean(body.carModel) || null},${clean(body.color) || null},${clean(body.financeType) || null},${clean(body.location) || null},${clean(body.notes) || null},${assignedTo}::uuid,${callCenterTo}::uuid,${user.id}::uuid,${user.id}::uuid,now(),${completionPercent}) returning *,id::text,assigned_to::text,call_center_assigned_to::text
      `;
      await sql`update crm.manual_lead_requests set created_lead_id=${createdLead.id}::uuid,reviewed_by=${user.id}::uuid,reviewed_at=now(),updated_at=now() where id=${requestRow.id}::uuid`;
      await attachLeadToContactAndOpenRequest({ leadId: createdLead.id, actor: user, classificationMethod: "manual" });
      await sql`insert into crm.lead_events(lead_id,event_type,new_status,new_department,new_branch,actor_id,actor_name,note) values (${createdLead.id}::uuid,'manual_lead_created','عميل جديد',${departmentCode},${branchCode},${user.id}::uuid,${user.fullName},'إضافة عميل يدوي')`;
    }
    await audit(user, "manual_lead_requested", "manual_lead_request", requestRow.id, requestRow);
    if (createdLead) {
      createdLead.customer_name = customerName;
      createdLead.assigned_to = assignedTo;
      createdLead.call_center_assigned_to = callCenterTo;
      createdLead.branch_code = branchCode;
      createdLead.department_code = departmentCode;
      await emitCrmLeadNotification(user, "created", createdLead).catch((error) => console.error("CRM manual lead notification failed", error));
    }
  ...
  if (request.method === "PATCH") {
    ...
    if (action === "edit") {
      ...
      let newlyCreatedLead: any = null;
      if (!row.created_lead_id && !duplicate) {
        const customerFields = await getCustomerFieldDefinitions();
        const completionPercent = calculateLeadCompletion({ customerName, phone, sourceCode, statusLabel: "عميل جديد", serviceKey, location: clean(body.location), carName: clean(body.carName), carCategory: clean(body.carCategory), carModel: clean(body.carModel), color: clean(body.color), financeType: clean(body.financeType) }, customerFields);
        [newlyCreatedLead] = await sql<any[]>`
          insert into crm.leads(customer_name,phone,phone_normalized,source_code,source_name,service_key,department_code,branch_code,status_label,payment_type,car_name,car_category,car_model,color,finance_type,location,notes,assigned_to,call_center_assigned_to,created_by,updated_by,registered_at,completion_percent)
          values (${customerName},${phone},${phoneNormalized},${sourceCode},${sourceName},${serviceKey},${departmentCode},${branchCode || null},'عميل جديد',${paymentType},${clean(body.carName) || null},${clean(body.carCategory) || null},${clean(body.carModel) || null},${clean(body.color) || null},${clean(body.financeType) || null},${clean(body.location) || null},${clean(body.notes) || null},${assignedTo}::uuid,${callCenterTo}::uuid,${user.id}::uuid,${user.id}::uuid,coalesce(${row.registered_at}::timestamptz,now()),${completionPercent}) returning *,id::text,assigned_to::text,call_center_assigned_to::text
        `;
        await sql`update crm.manual_lead_requests set approval_status='approved',created_lead_id=${newlyCreatedLead.id}::uuid,reviewed_by=${user.id}::uuid,reviewed_at=now(),updated_at=now() where id=${id}::uuid`;
        await attachLeadToContactAndOpenRequest({ leadId: newlyCreatedLead.id, actor: user, classificationMethod: "manual_edit" });
      }
      ...
      if (newlyCreatedLead) {
        newlyCreatedLead.customer_name = customerName;
        newlyCreatedLead.assigned_to = assignedTo;
        newlyCreatedLead.call_center_assigned_to = callCenterTo;
        newlyCreatedLead.branch_code = branchCode;
        newlyCreatedLead.department_code = departmentCode;
        await emitCrmLeadNotification(user, "created", newlyCreatedLead).catch((error) => console.error("CRM manual lead notification failed", error));
      }
