...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    return response.status(404).json({ ok: false, error: "Zoho route not found" });
  } catch (failure) {
    return response.status(400).json({ ok: false, error: failure instanceof Error ? failure.message : "تعذر تنفيذ طلب Zoho" });
  }
}
