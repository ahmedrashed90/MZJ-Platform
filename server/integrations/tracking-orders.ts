...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  const sql = getSql();
  const [blocked] = await sql<{ is_blocked: boolean; reason: string | null }[]>`
    select is_blocked,reason from tracking.deleted_order_blocks where sales_order_no=${orderNo}
  `;
  if (blocked?.is_blocked) {
    return response.status(409).json({
      ok: false,
      blocked: true,
      error: "هذا الطلب محذوف ومحظور من إعادة الاستقبال حتى تسمح الإدارة بإرساله مرة أخرى",
    ...
    const result = await sql.begin(async (tx) => {
      const [order] = await tx<any[]>`
        insert into tracking.orders(
          ...
          ${numberValue(totals.registrationFee)},${clean(body.integrationSource)||"google-sheets-next-erp"},${tx.json(body)},now(),false,now()
        )
      ...
      let [vehicle] = await tx<any[]>`
        ...
        where order_id=${order.id}::uuid
          and ((item_no is not null and item_no=${itemNo}) or vin=${vin})
        order by case when item_no=${itemNo} then 0 else 1 end
        limit 1
      `;
      ...
      await tx`
        update tracking.orders o set
          subtotal_before_tax=coalesce(x.subtotal_before_tax,0),
          tax_value=coalesce(x.tax_value,0),
          total_incl_vat=coalesce(x.total_incl_vat,0),
          registration_fee=coalesce(x.registration_fee,0),
          updated_at=now()
        from (
          select order_id,
            sum(subtotal_excl_vat)+max(registration_fee) as subtotal_before_tax,
            sum(tax_value) as tax_value,
            sum(total_incl_vat)+max(registration_fee) as total_incl_vat,
      ...
      await tx`
        ...
        values ('google-sheets-next-erp',${`${orderNo}:${itemNo}`},'tracking_order_upsert',${tx.json(body)},'processed',now())
