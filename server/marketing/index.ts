...
function projectScope(sql: ReturnType<typeof getSql>, user: SessionUser, alias = "c") {
  ...
  return sql`(
    ...
    or exists(select 1 from marketing_native.tasks st where st.campaign_id=${sql.unsafe(alias)}.id and (st.assigned_to=${user.id}::uuid or st.content_writer_id=${user.id}::uuid))
  )`;
}
...
async function loadMeta(sql: ReturnType<typeof getSql>, user: SessionUser) {
  const [departments, departmentUsers, actions, creativeTypes, campaignTypes, platforms, postTypes, categories, requestStatuses, users, attendanceSettings] = await Promise.all([
    sql`select id::text,code,name,is_content_department,is_active,sort_order from marketing_native.departments order by sort_order,name`,
    sql`select du.department_id::text,du.user_id::text,u.full_name,u.email,du.is_active from marketing_native.department_users du join core.users u on u.id=du.user_id where u.is_active=true order by u.full_name`,
    sql`select a.id::text,a.department_id::text,d.code as department_code,d.name as department_name,a.name,a.percentage::float,a.audience,a.is_required,a.is_active,a.sort_order from marketing_native.assignment_actions a join marketing_native.departments d on d.id=a.department_id order by d.sort_order,a.sort_order,a.name`,
    sql`select ct.id::text,ct.name,ct.short_code,ct.primary_department_id::text,d.code as primary_department_code,d.name as primary_department_name,ct.is_active,ct.sort_order from marketing_native.creative_types ct left join marketing_native.departments d on d.id=ct.primary_department_id order by ct.sort_order,ct.name`,
    sql`select id::text,name,short_code,code_prefix,is_active,sort_order,next_number::text from marketing_native.campaign_types order by sort_order,name`,
    sql`select id::text,code,name,is_active,sort_order from marketing_native.platforms order by sort_order,name`,
    sql`select pt.id::text,pt.platform_id::text,p.code as platform_code,p.name as platform_name,pt.name,pt.code,pt.dimensions,pt.is_active,pt.sort_order from marketing_native.platform_post_types pt join marketing_native.platforms p on p.id=pt.platform_id order by p.sort_order,pt.sort_order,pt.name`,
    sql`select id::text,name,is_active,sort_order from marketing_native.package_categories order by sort_order,name`,
    sql`select id::text,code,name,is_terminal,is_active,sort_order from marketing_native.request_statuses order by sort_order,name`,
    ...
    sql`select work_start_time::text,work_end_time::text,grace_minutes,idle_after_minutes,offline_after_minutes,updated_at from marketing_native.attendance_settings order by updated_at desc nulls last limit 1`,
  ]);
...
async function dashboard(sql: ReturnType<typeof getSql>, user: SessionUser) {
  ...
  const [counts] = await sql`
    ...
    from marketing_native.campaigns c where ${scope}
  `;
  const taskRows = await sql`
    select t.id::text,t.task_no,t.task_kind,t.status,t.review_status,t.review_note,t.template_data,t.final_file_name,t.final_file_url,t.progress::float,t.due_at,t.received_at,t.completed_at,
      ...
      (select ia.notes from marketing_native.instance_assignments ia where ia.creative_id=t.creative_id and ia.department_id=t.department_id and ia.assigned_user_id=t.assigned_to and ia.content_writer_id=t.content_writer_id limit 1) as assignment_notes,
      coalesce((select json_agg(json_build_object('id',v.id::text,'vin',v.vin,'car_name',v.car_name,'statement',v.statement,'exterior_color',v.exterior_color,'interior_color',v.interior_color,'model_year',v.model_year,'location_name',l.name) order by v.vin) from marketing_native.instance_vehicles iv join operations.vehicles v on v.id=iv.vehicle_id left join operations.locations l on l.id=v.location_id where iv.creative_id=t.creative_id),'[]') as vehicles,
      coalesce((select json_agg(json_build_object('id',a.id::text,'name',a.name,'percentage',a.percentage::float,'completed',coalesce(ap.completed,false),'completed_at',ap.completed_at,'note',ap.note) order by a.sort_order)
        from marketing_native.assignment_actions a left join marketing_native.task_action_progress ap on ap.action_id=a.id and ap.task_id=t.id
      ...
      coalesce((select json_agg(json_build_object('id',u.id::text,'upload_kind',u.upload_kind,'file_name',u.file_name,'external_url',u.external_url,'version_no',u.version_no,'created_at',u.created_at) order by u.created_at desc) from marketing_native.task_uploads u where u.task_id=t.id),'[]') as uploads
    from marketing_native.tasks t
    join marketing_native.campaigns c on c.id=t.campaign_id
    left join marketing_native.creatives cr on cr.id=t.creative_id
    left join marketing_native.departments d on d.id=t.department_id
  ...
  const projects = await sql`
    select c.id::text,c.name,c.campaign_code,c.source_kind,c.status,c.stage,c.starts_on,c.ends_on,c.moved_to_publish_at,c.created_at,
      ...
      coalesce((select round(avg(dept_progress),2) from (
        select avg(tx.progress)::numeric as dept_progress from marketing_native.tasks tx where tx.campaign_id=c.id and tx.task_kind='execution' group by tx.department_id
      ) q),case when count(t.id)=0 then 0 else round(avg(t.progress),2) end,0)::float as progress
    from marketing_native.campaigns c left join marketing_native.tasks t on t.campaign_id=c.id
...
async function listProjects(sql: ReturnType<typeof getSql>, request: VercelRequest, user: SessionUser) {
  ...
  const rows = await sql`
    select c.id::text,c.campaign_code,c.name,c.source_kind,c.campaign_type,c.objective,c.content_brief,c.status,c.stage,c.campaign_date,c.starts_on,c.ends_on,
      ...
      coalesce((select round(avg(dept_progress),2) from (select avg(tx.progress)::numeric as dept_progress from marketing_native.tasks tx where tx.campaign_id=c.id and tx.task_kind='execution' group by tx.department_id) q),0)::float as progress
    from marketing_native.campaigns c left join core.users u on u.id=c.created_by left join marketing_native.creatives cr on cr.campaign_id=c.id left join marketing_native.tasks t on t.campaign_id=c.id
...
async function projectDetail(sql: ReturnType<typeof getSql>, id: string, user: SessionUser) {
  ...
  const [project] = await sql`
    ...
    from marketing_native.campaigns c left join marketing_native.campaign_types ct on ct.id=c.campaign_type_id left join core.users u on u.id=c.created_by
  ...
  const [creatives, assignments, vehicles, tasks, budget, schedule, links, files, activity] = await Promise.all([
    sql`select cr.*,cr.id::text,cr.campaign_id::text,cr.creative_type_id::text,ct.name as creative_type_name,ct.short_code as catalog_short_code,d.name as primary_department_name from marketing_native.creatives cr left join marketing_native.creative_types ct on ct.id=cr.creative_type_id left join marketing_native.departments d on d.id=ct.primary_department_id where cr.campaign_id=${id}::uuid order by cr.sort_order,cr.instance_no`,
    sql`select a.id::text,a.creative_id::text,a.department_id::text,a.assigned_user_id::text,a.content_writer_id::text,a.assignment_role,a.due_at,a.notes,a.is_optional,d.name as department_name,d.code as department_code,u.full_name as assigned_name,cw.full_name as content_writer_name from marketing_native.instance_assignments a join marketing_native.departments d on d.id=a.department_id join core.users u on u.id=a.assigned_user_id left join core.users cw on cw.id=a.content_writer_id where a.creative_id in(select id from marketing_native.creatives where campaign_id=${id}::uuid) order by d.sort_order,u.full_name,cw.full_name`,
    sql`select iv.creative_id::text,v.id::text as vehicle_id,v.vin,v.car_name,v.statement,v.exterior_color,v.interior_color,v.model_year,l.name as location_name from marketing_native.instance_vehicles iv join operations.vehicles v on v.id=iv.vehicle_id left join operations.locations l on l.id=v.location_id where iv.creative_id in(select id from marketing_native.creatives where campaign_id=${id}::uuid) order by v.vin`,
    sql`select t.*,t.id::text,t.campaign_id::text,t.creative_id::text,t.assigned_to::text,t.content_writer_id::text,t.template_task_id::text,t.department_id::text,au.full_name as assigned_name,cw.full_name as content_writer_name,d.name as department_name,c.name as project_name,c.campaign_code,c.source_kind,c.campaign_type,c.starts_on as project_starts_on,c.ends_on as project_ends_on,c.objective as project_objective,c.content_brief as project_content_brief,c.stage as project_stage,cr.instance_no,cr.creative_type,cr.short_code,cr.agenda_day,cr.content_due_at,cr.content_notes,cr.admin_notes,(select ia.notes from marketing_native.instance_assignments ia where ia.creative_id=t.creative_id and ia.department_id=t.department_id and ia.assigned_user_id=t.assigned_to and ia.content_writer_id=t.content_writer_id limit 1) as assignment_notes,coalesce((select json_agg(json_build_object('id',v.id::text,'vin',v.vin,'car_name',v.car_name,'statement',v.statement,'exterior_color',v.exterior_color,'interior_color',v.interior_color,'model_year',v.model_year,'location_name',l.name) order by v.vin) from marketing_native.instance_vehicles iv join operations.vehicles v on v.id=iv.vehicle_id left join operations.locations l on l.id=v.location_id where iv.creative_id=t.creative_id),'[]') as vehicles,coalesce((select json_agg(json_build_object('id',a.id::text,'name',a.name,'percentage',a.percentage::float,'completed',coalesce(ap.completed,false),'completed_at',ap.completed_at,'note',ap.note) order by a.sort_order) from marketing_native.assignment_actions a left join marketing_native.task_action_progress ap on ap.action_id=a.id and ap.task_id=t.id where a.department_id=t.department_id and a.is_active=true and (${showAdminActions}=true or a.audience in ('user','both'))),'[]') as actions,coalesce((select json_agg(json_build_object('id',up.id::text,'upload_kind',up.upload_kind,'file_name',up.file_name,'external_url',up.external_url,'storage_key',up.storage_key,'version_no',up.version_no,'uploaded_by_name',up.uploaded_by_name,'created_at',up.created_at) order by up.created_at desc) from marketing_native.task_uploads up where up.task_id=t.id),'[]') as uploads from marketing_native.tasks t join marketing_native.campaigns c on c.id=t.campaign_id left join core.users au on au.id=t.assigned_to left join core.users cw on cw.id=t.content_writer_id left join marketing_native.departments d on d.id=t.department_id left join marketing_native.creatives cr on cr.id=t.creative_id where t.campaign_id=${id}::uuid order by cr.sort_order,t.task_kind desc,t.created_at`,
    sql`select b.id::text,b.creative_id::text,b.platform_id::text,b.funnel,b.ad_count,b.content_goal,b.expected_goal,b.amount::float,b.notes,cr.instance_no,cr.creative_type,p.name as platform_name from marketing_native.budget_items b left join marketing_native.creatives cr on cr.id=b.creative_id left join marketing_native.platforms p on p.id=b.platform_id where b.campaign_id=${id}::uuid order by b.sort_order,b.created_at`,
    sql`select s.id::text,s.creative_id::text,s.platform_id::text,s.post_type_id::text,s.publish_date,s.publish_time::text,s.notes,s.status,cr.instance_no,cr.creative_type,p.name as platform_name,pt.name as post_type_name,pt.dimensions from marketing_native.publish_schedule s join marketing_native.creatives cr on cr.id=s.creative_id join marketing_native.platforms p on p.id=s.platform_id join marketing_native.platform_post_types pt on pt.id=s.post_type_id where s.campaign_id=${id}::uuid order by s.publish_date,s.publish_time,cr.instance_no`,
    sql`select l.id::text,l.platform_id::text,p.name as platform_name,l.url,l.created_at from marketing_native.project_links l left join marketing_native.platforms p on p.id=l.platform_id where l.campaign_id=${id}::uuid order by l.created_at desc`,
    sql`select f.id::text,f.file_kind,f.file_name,f.external_url,f.storage_key,f.mime_type,f.file_size,f.uploaded_by_name,f.created_at from marketing_native.project_files f where f.campaign_id=${id}::uuid order by f.created_at desc`,
    sql`select id::text,actor_name,action,entity_type,entity_id,details,created_at from marketing_native.activity_log where entity_id=${id} order by created_at desc limit 100`,
  ]);
...
async function nextProjectCode(tx: SqlExecutor, kind: string, campaignTypeId: string | null, startDate: string) {
  if (kind === "agenda") {
    const [sequence] = await tx`select nextval('marketing_native.project_code_seq')::bigint as n`;
  ...
  if (campaignTypeId) {
    const [row] = await tx`select id::text,short_code,code_prefix,next_number::bigint as next_number from marketing_native.campaign_types where id=${campaignTypeId}::uuid and is_active=true for update`;
    ...
    await tx`update marketing_native.campaign_types set next_number=next_number+1,updated_at=now() where id=${row.id}::uuid`;
...
async function createProject(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  return sql.begin(async (tx) => {
    const [existing] = await tx`select id::text,campaign_code,name from marketing_native.campaigns where idempotency_key=${idempotencyKey}`;
    ...
    const [campaignType] = campaignTypeId ? await tx`select name from marketing_native.campaign_types where id=${campaignTypeId}::uuid` : [null];
    const [project] = await tx`
      insert into marketing_native.campaigns(campaign_code,name,campaign_type,campaign_type_id,objective,content_brief,status,source_kind,stage,campaign_date,starts_on,ends_on,starts_at,ends_at,created_by,idempotency_key,metadata)
    ...
    for (const raw of instances) {
      ...
      const [creativeType] = await tx`select ct.id::text,ct.name,ct.short_code,ct.primary_department_id::text,d.code as primary_department_code from marketing_native.creative_types ct left join marketing_native.departments d on d.id=ct.primary_department_id where ct.id=${creativeTypeId}::uuid and ct.is_active=true`;
      ...
      const [creative] = await tx`
        insert into marketing_native.creatives(campaign_id,creative_type,creative_type_id,quantity,status,instance_no,short_code,agenda_day,content_due_at,content_notes,admin_notes,sort_order,metadata)
      ...
      for (const vehicleId of vehicleIds) {
        await tx`insert into marketing_native.instance_vehicles(creative_id,vehicle_id) select ${creative.id}::uuid,v.id from operations.vehicles v where v.id=${vehicleId}::uuid and v.is_deleted=false on conflict do nothing`;
      }
      ...
      const [contentDepartment] = await tx`select id::text,code from marketing_native.departments where is_content_department=true and is_active=true limit 1`;
      ...
      for (const writerId of contentWriterIds) {
        const [writer] = await tx`select u.id::text,u.full_name from core.users u join marketing_native.department_users du on du.user_id=u.id and du.department_id=${contentDepartment.id}::uuid and du.is_active=true where u.id=${writerId}::uuid and u.is_active=true`;
        if (!writer) throw new Error("كاتب المحتوى المختار غير موجود");
        await tx`insert into marketing_native.instance_assignments(creative_id,department_id,assigned_user_id,content_writer_id,assignment_role,due_at,notes,is_optional) values(${creative.id}::uuid,${contentDepartment.id}::uuid,${writerId}::uuid,${writerId}::uuid,'content',${textOrNull(raw.contentDueAt)}::timestamptz,${textOrNull(raw.contentNotes)},false) on conflict do nothing`;
        const [seq] = await tx`select nextval('marketing_native.task_no_seq')::bigint as n`;
        const taskNo = `${instanceNo}-${creativeType.short_code}-CONTENT-${String(seq?.n || 1).padStart(7, "0")}`;
        const [task] = await tx`
          insert into marketing_native.tasks(task_no,campaign_id,creative_id,task_kind,department_id,department_code,assigned_to,content_writer_id,paired_content_user_id,status,due_at,review_status,metadata)
      ...
      for (const assignment of assignments) {
        ...
        const [department] = await tx`select id::text,code,name from marketing_native.departments where id=${departmentId}::uuid and is_active=true`;
        const [assignee] = await tx`select u.id::text,u.full_name from core.users u join marketing_native.department_users du on du.user_id=u.id and du.department_id=${departmentId}::uuid and du.is_active=true where u.id=${userId}::uuid and u.is_active=true`;
        ...
        for (const writerId of writerIds) {
          ...
          const [task] = await tx`
            insert into marketing_native.tasks(task_no,campaign_id,creative_id,task_kind,department_id,department_code,assigned_to,content_writer_id,paired_content_user_id,template_task_id,status,due_at,review_status,metadata)
          ...
          `;
          await tx`
            insert into marketing_native.task_action_progress(task_id,action_id)
            select ${task.id}::uuid,a.id from marketing_native.assignment_actions a where a.department_id=${department.id}::uuid and a.is_active=true
    ...
    for (const raw of list(body.budget).map(objectValue)) {
      ...
      const [platform] = await tx`select id::text from marketing_native.platforms where id=${platformId}::uuid and is_active=true`;
      if (!platform) throw new Error("منصة بند الميزانية غير صحيحة أو غير فعالة");
      await tx`insert into marketing_native.budget_items(campaign_id,creative_id,funnel,platform_id,ad_count,content_goal,expected_goal,amount,notes,sort_order) values(${project.id}::uuid,${creativeId}::uuid,${clean(raw.funnel) || "عام"},${platform.id}::uuid,${Math.max(1, Math.round(numberValue(raw.adCount) || 1))},${textOrNull(raw.contentGoal)},${textOrNull(raw.expectedGoal)},${Math.max(0, numberValue(raw.amount))},${textOrNull(raw.notes)},${numberValue(raw.sortOrder)})`;
    }
    for (const raw of list(body.schedule).map(objectValue)) {
      ...
      const [postType] = await tx`select pt.id::text,pt.platform_id::text from marketing_native.platform_post_types pt join marketing_native.platforms p on p.id=pt.platform_id where pt.id=${postTypeId}::uuid and pt.platform_id=${platformId}::uuid and pt.is_active=true and p.is_active=true`;
      if (!postType) throw new Error("نوع النشر لا يتبع المنصة المختارة أو غير فعال");
      await tx`insert into marketing_native.publish_schedule(campaign_id,creative_id,publish_date,publish_time,platform_id,post_type_id,notes) values(${project.id}::uuid,${creativeId}::uuid,${publishDate}::date,${textOrNull(raw.publishTime)}::time,${platformId}::uuid,${postType.id}::uuid,${textOrNull(raw.notes)}) on conflict do nothing`;
    }
    await tx`insert into marketing_native.activity_log(actor_id,actor_name,action,entity_type,entity_id,details) values(${user.id}::uuid,${user.fullName},'project_created','project',${project.id},${tx.json(toDatabaseJson({ kind, code, name, instances: instances.length }))})`;
...
async function receiveTask(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  return sql.begin(async (tx) => {
    const [task] = await tx`select *,id::text,assigned_to::text,template_task_id::text from marketing_native.tasks where id=${id}::uuid for update`;
    ...
    if (task.task_kind === "execution" && task.template_task_id) {
      const [template] = await tx`select review_status from marketing_native.tasks where id=${task.template_task_id}::uuid`;
    ...
    const [updated] = await tx`update marketing_native.tasks set received_at=now(),received_by=${user.id}::uuid,status='active',updated_at=now() where id=${id}::uuid returning *,id::text`;
    await tx`insert into marketing_native.activity_log(actor_id,actor_name,action,entity_type,entity_id,details) values(${user.id}::uuid,${user.fullName},'task_received','task',${id},${tx.json(toDatabaseJson({ receivedAt: updated.received_at }))})`;
...
async function submitTemplate(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  return sql.begin(async (tx) => {
    const [task] = await tx`select *,id::text,assigned_to::text from marketing_native.tasks where id=${id}::uuid and task_kind='template' for update`;
    ...
    const [updated] = await tx`update marketing_native.tasks set template_data=${tx.json(toDatabaseJson(templateData))},review_status='pending_review',status='review',review_note=null,progress=100,updated_at=now() where id=${id}::uuid returning *,id::text`;
    await tx`insert into marketing_native.task_reviews(task_id,action,note,reviewer_id,reviewer_name,snapshot) values(${id}::uuid,'submitted',${textOrNull(body.note)},${user.id}::uuid,${user.fullName},${tx.json(toDatabaseJson(templateData))})`;
    await tx`insert into marketing_native.activity_log(actor_id,actor_name,action,entity_type,entity_id,details) values(${user.id}::uuid,${user.fullName},'template_submitted','task',${id},${tx.json(toDatabaseJson({ fields: Object.keys(templateData) }))})`;
...
async function reviewTemplate(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  return sql.begin(async (tx) => {
    const [task] = await tx`select *,id::text from marketing_native.tasks where id=${id}::uuid and task_kind='template' for update`;
    if (!task) throw new Error("Task Template غير موجودة");
    if (reviewAction === "approve") {
      await tx`update marketing_native.tasks set review_status='approved',review_note=${note || null},status='completed',completed_at=coalesce(completed_at,now()),progress=100,updated_at=now() where id=${id}::uuid`;
      await tx`update marketing_native.tasks set template_data=${tx.json(toDatabaseJson(task.template_data || {}))},review_status='approved',status=case when received_at is null then 'required' else 'active' end,updated_at=now() where template_task_id=${id}::uuid`;
    } else if (reviewAction === "request_revision") {
      await tx`update marketing_native.tasks set review_status='revision_requested',review_note=${note || null},status='revision_requested',completed_at=null,updated_at=now() where id=${id}::uuid`;
      await tx`update marketing_native.tasks set review_status='waiting_template',status='waiting_template',template_data='{}'::jsonb,updated_at=now() where template_task_id=${id}::uuid`;
    } else {
      await tx`update marketing_native.tasks set review_status='rejected',review_note=${note || null},status='rejected',completed_at=null,updated_at=now() where id=${id}::uuid`;
      await tx`update marketing_native.tasks set review_status='rejected',status='waiting_template',template_data='{}'::jsonb,updated_at=now() where template_task_id=${id}::uuid`;
    }
    const dbAction = reviewAction === "approve" ? "approved" : reviewAction === "request_revision" ? "revision_requested" : "rejected";
    await tx`insert into marketing_native.task_reviews(task_id,action,note,reviewer_id,reviewer_name,snapshot) values(${id}::uuid,${dbAction},${note || null},${user.id}::uuid,${user.fullName},${tx.json(toDatabaseJson(task.template_data || {}))})`;
    await tx`insert into marketing_native.activity_log(actor_id,actor_name,action,entity_type,entity_id,details) values(${user.id}::uuid,${user.fullName},${`template_${dbAction}`},'task',${id},${tx.json(toDatabaseJson({ note }))})`;
...
async function taskAction(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  return sql.begin(async (tx) => {
    const [task] = await tx`select *,id::text,assigned_to::text from marketing_native.tasks where id=${taskId}::uuid and task_kind='execution' for update`;
    ...
    const [assignmentAction] = await tx`select id::text,audience from marketing_native.assignment_actions where id=${actionId}::uuid and department_id=${task.department_id}::uuid and is_active=true`;
    ...
    const [updated] = await tx`update marketing_native.tasks set progress=${progress},status=${status},completed_at=${status === "completed" ? new Date().toISOString() : null}::timestamptz,updated_at=now() where id=${taskId}::uuid returning *,id::text`;
...
async function moveToPublish(sql: ReturnType<typeof getSql>, id: string, user: SessionUser) {
  return sql.begin(async (tx) => {
    const [progress] = await tx`select coalesce((select round(avg(dept_progress),2) from (select avg(t.progress)::numeric as dept_progress from marketing_native.tasks t where t.campaign_id=${id}::uuid and t.task_kind='execution' group by t.department_id) q),0)::float as value`;
    if (Number(progress?.value || 0) < 99.99) throw new Error("لا يمكن النقل إلى قسم النشر قبل وصول كل الأقسام إلى 100%");
    const [project] = await tx`update marketing_native.campaigns set stage='publishing',moved_to_publish_at=now(),updated_at=now() where id=${id}::uuid and is_deleted=false returning id::text,name,campaign_code,stage`;
    if (!project) throw new Error("الحملة أو الأجندة غير موجودة");
    await tx`insert into marketing_native.activity_log(actor_id,actor_name,action,entity_type,entity_id,details) values(${user.id}::uuid,${user.fullName},'moved_to_publishing','project',${id},'{}'::jsonb)`;
...
async function projectStateAction(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  if (stateAction === "archive") {
    const [row] = await sql`update marketing_native.campaigns set archived_at=now(),archived_by=${user.id}::uuid,updated_at=now() where id=${id}::uuid and is_deleted=false returning id::text`;
    if (!row) throw new Error("السجل غير موجود");
    return { ok: true, message: "تمت الأرشفة" };
  }
  if (stateAction === "restore") {
    const [row] = await sql`update marketing_native.campaigns set archived_at=null,archived_by=null,updated_at=now() where id=${id}::uuid and is_deleted=false returning id::text`;
    if (!row) throw new Error("السجل غير موجود");
    return { ok: true, message: "تم الاسترجاع من الأرشيف" };
  }
  if (stateAction === "delete") {
    const [row] = await sql`update marketing_native.campaigns set is_deleted=true,deleted_at=now(),deleted_by=${user.id}::uuid,updated_at=now() where id=${id}::uuid and is_deleted=false returning id::text`;
    if (!row) throw new Error("السجل غير موجود");
...
async function updateProject(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  if (endsOn < startsOn) throw new Error("تاريخ النهاية يجب ألا يسبق البداية");
  const [current] = await sql`select id::text,source_kind from marketing_native.campaigns where id=${id}::uuid and is_deleted=false`;
  if (!current) throw new Error("الحملة أو الأجندة غير موجودة");
  const [campaignType] = campaignTypeId ? await sql`select id::text,name from marketing_native.campaign_types where id=${campaignTypeId}::uuid and is_active=true` : [null];
  if (campaignTypeId && !campaignType) throw new Error("نوع الحملة غير صحيح أو غير فعال");
  const [row] = await sql`
    update marketing_native.campaigns set
  ...
  `;
  await sql`insert into marketing_native.activity_log(actor_id,actor_name,action,entity_type,entity_id,details) values(${user.id}::uuid,${user.fullName},'project_updated','project',${id},${sql.json(toDatabaseJson({ name, startsOn, endsOn, campaignTypeId }))})`;
...
async function saveProjectLink(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  const [row] = await sql`insert into marketing_native.project_links(campaign_id,platform_id,url,created_by) values(${campaignId}::uuid,${textOrNull(body.platformId)}::uuid,${url},${user.id}::uuid) returning id::text,url,created_at`;
...
async function packages(sql: ReturnType<typeof getSql>, request: VercelRequest) {
  ...
  const [rows, countRows] = await Promise.all([
    sql`select p.id::text,p.name,p.category_id::text,c.name as category_name,p.price::float,p.cash_discount_percent::float,p.registration_fee::float,p.insurance_fee::float,p.issuance_fee::float,p.care_items,p.delivery_home,p.delivery_region,p.metadata,p.is_active,p.created_at,p.updated_at from marketing_native.packages p left join marketing_native.package_categories c on c.id=p.category_id where p.is_active=true and (${categoryId}='' or p.category_id=${categoryId}::uuid) and (${search}='' or p.name ilike ${pattern} or coalesce(c.name,'') ilike ${pattern}) order by c.sort_order,p.name`,
    sql`select p.category_id::text as category_id,count(*)::int as count from marketing_native.packages p left join marketing_native.package_categories c on c.id=p.category_id where p.is_active=true and (${search}='' or p.name ilike ${pattern} or coalesce(c.name,'') ilike ${pattern}) group by p.category_id`,
  ]);
...
async function savePackage(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  const [row] = id ? await sql`update marketing_native.packages set name=${name},category_id=${categoryId}::uuid,price=${values.price},cash_discount_percent=${values.discount},registration_fee=${values.registration},insurance_fee=${values.insurance},issuance_fee=${values.issuance},care_items=${values.careItems},delivery_home=${values.deliveryHome},delivery_region=${values.deliveryRegion},metadata=${sql.json(toDatabaseJson(values.metadata))},updated_at=now() where id=${id}::uuid returning id::text` : await sql`insert into marketing_native.packages(name,category_id,price,cash_discount_percent,registration_fee,insurance_fee,issuance_fee,care_items,delivery_home,delivery_region,metadata,created_by) values(${name},${categoryId}::uuid,${values.price},${values.discount},${values.registration},${values.insurance},${values.issuance},${values.careItems},${values.deliveryHome},${values.deliveryRegion},${sql.json(toDatabaseJson(values.metadata))},${user.id}::uuid) returning id::text`;
...
async function deletePackage(sql: ReturnType<typeof getSql>, id: string) {
  const [row] = await sql`update marketing_native.packages set is_active=false,updated_at=now() where id=${id}::uuid returning id::text`;
...
async function stock(sql: ReturnType<typeof getSql>, request: VercelRequest) {
  ...
  const rows = await sql`
    select v.id::text,v.vin,v.car_name,v.statement,v.interior_color,v.exterior_color,v.model_year,v.status_code,s.name as status_name,l.name as location_name,l.code as location_code,
      exists(select 1 from operations.photography_request_vehicles prv join operations.photography_requests pr on pr.id=prv.request_id left join marketing_native.request_statuses rs on rs.code=pr.status where prv.vehicle_id=v.id and pr.is_deleted=false and not coalesce(rs.is_terminal,false)) as has_active_photo_request
...
async function createPhotoRequest(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  return sql.begin(async (tx) => {
    ...
    const [active] = await tx`select r.request_no,v.vin from operations.photography_request_vehicles rv join operations.photography_requests r on r.id=rv.request_id join operations.vehicles v on v.id=rv.vehicle_id left join marketing_native.request_statuses rs on rs.code=r.status where rv.vehicle_id in ${tx(vehicleIds)} and r.is_deleted=false and not coalesce(rs.is_terminal,false) limit 1`;
    if (active) throw new Error(`السيارة ${active.vin} مرتبطة بطلب تصوير نشط ${active.request_no}`);
    const [seq] = await tx`select nextval('marketing_native.photo_request_no_seq')::bigint as n`;
    ...
    await tx`insert into marketing_native.activity_log(actor_id,actor_name,action,entity_type,entity_id,details) values(${user.id}::uuid,${user.fullName},'photo_request_created','photography_request',${request.id},${tx.json(toDatabaseJson({ requestNo, vehicleIds }))})`;
...
async function updatePhotoRequest(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  const [validStatus] = await sql`select code,is_terminal from marketing_native.request_statuses where code=${status} and is_active=true`;
  ...
  await sql`insert into marketing_native.activity_log(actor_id,actor_name,action,entity_type,entity_id,details) values(${user.id}::uuid,${user.fullName},'photo_request_updated','photography_request',${id},${sql.json(toDatabaseJson({ status, note: textOrNull(body.note) }))})`;
...
async function calendar(sql: ReturnType<typeof getSql>, request: VercelRequest, user: SessionUser) {
  ...
  const rows = await sql`
    ...
    from marketing_native.tasks t join marketing_native.campaigns c on c.id=t.campaign_id left join marketing_native.creatives cr on cr.id=t.creative_id left join core.users u on u.id=t.assigned_to left join marketing_native.departments d on d.id=t.department_id
  ...
  `;
  const publishRows = await sql`select s.id::text,s.publish_date,s.publish_time::text,c.id::text as campaign_id,c.name as project_name,c.campaign_code,c.source_kind,cr.instance_no,cr.creative_type,p.name as platform_name,pt.name as post_type_name from marketing_native.publish_schedule s join marketing_native.campaigns c on c.id=s.campaign_id join marketing_native.creatives cr on cr.id=s.creative_id join marketing_native.platforms p on p.id=s.platform_id join marketing_native.platform_post_types pt on pt.id=s.post_type_id where to_char(s.publish_date,'YYYY-MM')=${month} and c.is_deleted=false and c.archived_at is null and ${scope} order by s.publish_date,s.publish_time`;
...
async function publishPrep(sql: ReturnType<typeof getSql>, request: VercelRequest, user: SessionUser) {
  ...
  const rows = await sql`
    select t.id::text,t.task_no,t.status,t.progress::float,t.final_file_name,t.final_file_url,t.completed_at,c.id::text as campaign_id,c.name as project_name,c.campaign_code,c.source_kind,c.stage,cr.instance_no,cr.creative_type,u.full_name as assigned_name,d.name as department_name,
      coalesce((select json_agg(distinct jsonb_build_object('platform_id',p.id::text,'platform_name',p.name,'post_type',pt.name,'publish_date',s.publish_date,'publish_time',s.publish_time::text)) from marketing_native.publish_schedule s join marketing_native.platforms p on p.id=s.platform_id join marketing_native.platform_post_types pt on pt.id=s.post_type_id where s.creative_id=t.creative_id),'[]') as publications,
      coalesce((select json_agg(json_build_object('id',up.id::text,'file_name',up.file_name,'external_url',up.external_url,'storage_key',up.storage_key,'created_at',up.created_at) order by up.created_at desc) from marketing_native.task_uploads up where up.task_id=t.id and up.upload_kind='final'),'[]') as files
    from marketing_native.tasks t join marketing_native.campaigns c on c.id=t.campaign_id left join marketing_native.creatives cr on cr.id=t.creative_id left join core.users u on u.id=t.assigned_to left join marketing_native.departments d on d.id=t.department_id
    where t.task_kind='execution' and c.is_deleted=false and c.archived_at is null and (${search}='' or c.name ilike ${pattern} or coalesce(c.campaign_code,'') ilike ${pattern} or coalesce(t.task_no,'') ilike ${pattern}) and (${status}='' or t.status=${status}) and (${platformId}='' or exists(select 1 from marketing_native.publish_schedule sx where sx.creative_id=t.creative_id and sx.platform_id=${platformId}::uuid)) and ${scope}
...
async function reports(sql: ReturnType<typeof getSql>, request: VercelRequest, user: SessionUser) {
  ...
  const [summary] = await sql`
    ...
    from marketing_native.campaigns c left join marketing_native.tasks t on t.campaign_id=c.id left join marketing_native.budget_items b on b.campaign_id=c.id
  ...
  const departmentRows = await sql`
    ...
    from marketing_native.departments d left join marketing_native.tasks t on t.department_id=d.id left join marketing_native.campaigns c on c.id=t.campaign_id
  ...
  const userRows = await sql`
    ...
    from core.users u join marketing_native.tasks t on t.assigned_to=u.id join marketing_native.campaigns c on c.id=t.campaign_id left join marketing_native.departments d on d.id=t.department_id
    where c.is_deleted=false and c.archived_at is null and (${from}='' or c.created_at::date>=${from}::date) and (${to}='' or c.created_at::date<=${to}::date) and ${scope}
...
async function attendance(sql: ReturnType<typeof getSql>, request: VercelRequest, user: SessionUser) {
  ...
  const [settings] = await sql`select work_start_time::text,work_end_time::text,grace_minutes,idle_after_minutes,offline_after_minutes from marketing_native.attendance_settings order by updated_at desc nulls last limit 1`;
  const todayRows = await sql`select ar.id::text,ar.user_id::text,u.full_name,u.email,ar.attendance_date,ar.check_in_at,ar.check_out_at,ar.status,ar.late_minutes,ar.work_minutes,p.last_seen_at,p.last_activity_at,p.last_page from marketing_native.attendance_records ar join core.users u on u.id=ar.user_id left join marketing_native.presence_status p on p.user_id=ar.user_id where ar.attendance_date=${date}::date order by ar.check_in_at`;
  const reportRows = can(user, "marketing.attendance.manage") ? await sql`select ar.id::text,ar.user_id::text,u.full_name,u.email,ar.attendance_date,ar.check_in_at,ar.check_out_at,ar.status,ar.late_minutes,ar.work_minutes from marketing_native.attendance_records ar join core.users u on u.id=ar.user_id where ar.attendance_date between ${from}::date and ${to}::date order by ar.attendance_date desc,u.full_name` : await sql`select ar.id::text,ar.user_id::text,u.full_name,u.email,ar.attendance_date,ar.check_in_at,ar.check_out_at,ar.status,ar.late_minutes,ar.work_minutes from marketing_native.attendance_records ar join core.users u on u.id=ar.user_id where ar.user_id=${user.id}::uuid and ar.attendance_date between ${from}::date and ${to}::date order by ar.attendance_date desc`;
  const [myRecord] = await sql`select id::text,user_id::text,attendance_date,check_in_at,check_out_at,status,late_minutes,work_minutes from marketing_native.attendance_records where user_id=${user.id}::uuid and attendance_date=current_date`;
...
async function attendanceAction(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  if (attendanceAction === "presence") {
    await sql`insert into marketing_native.presence_status(user_id,last_seen_at,last_activity_at,last_page,activity_type,device_info,updated_at) values(${user.id}::uuid,now(),now(),${textOrNull(body.lastPage)},${textOrNull(body.activityType)},${sql.json(toDatabaseJson(body.deviceInfo))},now()) on conflict(user_id) do update set last_seen_at=now(),last_activity_at=now(),last_page=excluded.last_page,activity_type=excluded.activity_type,device_info=excluded.device_info,updated_at=now()`;
    return { ok: true };
  }
  const [settings] = await sql`select work_start_time::text,work_end_time::text,grace_minutes from marketing_native.attendance_settings order by updated_at desc nulls last limit 1`;
  if (attendanceAction === "check_in") {
    const [row] = await sql`
      insert into marketing_native.attendance_records(user_id,attendance_date,check_in_at,status,late_minutes,source,metadata)
      ...
      on conflict(user_id,attendance_date) do update set check_in_at=coalesce(marketing_native.attendance_records.check_in_at,excluded.check_in_at),status=case when marketing_native.attendance_records.check_in_at is null then excluded.status else marketing_native.attendance_records.status end,late_minutes=case when marketing_native.attendance_records.check_in_at is null then excluded.late_minutes else marketing_native.attendance_records.late_minutes end,updated_at=now()
...
async function connections(sql: ReturnType<typeof getSql>) {
  const rows = await sql`select p.id::text as platform_id,p.code,p.name,p.sort_order,c.id::text,c.connection_status,c.account_name,c.account_external_id,c.token_status,c.settings,c.connected_at,c.updated_at from marketing_native.platforms p left join marketing_native.platform_connections c on c.platform_id=p.id where p.is_active=true order by p.sort_order,p.name`;
  return { ok: true, rows };
}
...
async function settingsAction(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  if (entity === "department") {
    if (operation === "delete") { await sql`update marketing_native.departments set is_active=false,updated_at=now() where id=${id}::uuid`; return { ok: true, message: "تم تعطيل القسم" }; }
    ...
    if (bool(body.isContentDepartment)) await sql`update marketing_native.departments set is_content_department=false where is_content_department=true`;
    const [row] = id ? await sql`update marketing_native.departments set code=${code},name=${name},is_content_department=${bool(body.isContentDepartment)},is_active=${body.isActive !== false},sort_order=${numberValue(body.sortOrder)},updated_at=now() where id=${id}::uuid returning id::text` : await sql`insert into marketing_native.departments(code,name,is_content_department,is_active,sort_order) values(${code},${name},${bool(body.isContentDepartment)},${body.isActive !== false},${numberValue(body.sortOrder)}) returning id::text`;
  ...
  if (entity === "department_users") {
    ...
    await sql.begin(async (tx) => { await tx`delete from marketing_native.department_users where department_id=${departmentId}::uuid`; for (const userId of userIds) await tx`insert into marketing_native.department_users(department_id,user_id) values(${departmentId}::uuid,${userId}::uuid)`; });
    return { ok: true, message: "تم تحديث يوزرات القسم" };
  }
  if (entity === "action") {
    if (operation === "delete") { await sql`update marketing_native.assignment_actions set is_active=false,updated_at=now() where id=${id}::uuid`; return { ok: true, message: "تم تعطيل الإجراء" }; }
  ...
  if (entity === "creative_type") {
    ...
    const [row] = id ? await sql`update marketing_native.creative_types set name=${name},short_code=${shortCode},primary_department_id=${departmentId}::uuid,is_active=${body.isActive !== false},sort_order=${numberValue(body.sortOrder)},updated_at=now() where id=${id}::uuid returning id::text` : await sql`insert into marketing_native.creative_types(name,short_code,primary_department_id,is_active,sort_order) values(${name},${shortCode},${departmentId}::uuid,${body.isActive !== false},${numberValue(body.sortOrder)}) returning id::text`;
    return { ok: true, row, message: "تم حفظ الكرييتيف" };
  }
  if (entity === "campaign_type") {
    if (operation === "delete") { await sql`update marketing_native.campaign_types set is_active=false,updated_at=now() where id=${id}::uuid`; return { ok: true, message: "تم تعطيل نوع الحملة" }; }
    ...
    const [row] = id ? await sql`update marketing_native.campaign_types set name=${name},short_code=${shortCode},code_prefix=${prefix},is_active=${body.isActive !== false},sort_order=${numberValue(body.sortOrder)},updated_at=now() where id=${id}::uuid returning id::text` : await sql`insert into marketing_native.campaign_types(name,short_code,code_prefix,is_active,sort_order) values(${name},${shortCode},${prefix},${body.isActive !== false},${numberValue(body.sortOrder)}) returning id::text`;
    return { ok: true, row, message: "تم حفظ نوع الحملة" };
  }
  if (entity === "platform") {
    if (operation === "delete") { await sql`update marketing_native.platforms set is_active=false,updated_at=now() where id=${id}::uuid`; return { ok: true, message: "تم تعطيل المنصة" }; }
    const code = clean(body.code).toLowerCase().replace(/[^a-z0-9_]+/g, "_"); const name = clean(body.name);
    if (!code || !name) throw new Error("كود واسم المنصة مطلوبان");
    const [row] = id ? await sql`update marketing_native.platforms set code=${code},name=${name},is_active=${body.isActive !== false},sort_order=${numberValue(body.sortOrder)},updated_at=now() where id=${id}::uuid returning id::text` : await sql`insert into marketing_native.platforms(code,name,is_active,sort_order) values(${code},${name},${body.isActive !== false},${numberValue(body.sortOrder)}) returning id::text`;
    return { ok: true, row, message: "تم حفظ المنصة" };
  }
  if (entity === "post_type") {
    if (operation === "delete") { await sql`update marketing_native.platform_post_types set is_active=false,updated_at=now() where id=${id}::uuid`; return { ok: true, message: "تم تعطيل نوع النشر" }; }
    ...
    const [row] = id ? await sql`update marketing_native.platform_post_types set platform_id=${platformId}::uuid,name=${name},code=${code},dimensions=${textOrNull(body.dimensions)},is_active=${body.isActive !== false},sort_order=${numberValue(body.sortOrder)},updated_at=now() where id=${id}::uuid returning id::text` : await sql`insert into marketing_native.platform_post_types(platform_id,name,code,dimensions,is_active,sort_order) values(${platformId}::uuid,${name},${code},${textOrNull(body.dimensions)},${body.isActive !== false},${numberValue(body.sortOrder)}) returning id::text`;
    return { ok: true, row, message: "تم حفظ نوع النشر" };
  }
  if (entity === "category") {
    if (operation === "delete") { await sql`update marketing_native.package_categories set is_active=false,updated_at=now() where id=${id}::uuid`; return { ok: true, message: "تم تعطيل التصنيف" }; }
    const name = clean(body.name); if (!name) throw new Error("اسم التصنيف مطلوب");
    const [row] = id ? await sql`update marketing_native.package_categories set name=${name},is_active=${body.isActive !== false},sort_order=${numberValue(body.sortOrder)},updated_at=now() where id=${id}::uuid returning id::text` : await sql`insert into marketing_native.package_categories(name,is_active,sort_order) values(${name},${body.isActive !== false},${numberValue(body.sortOrder)}) returning id::text`;
    return { ok: true, row, message: "تم حفظ تصنيف الباقة" };
  }
  if (entity === "request_status") {
    if (operation === "delete") { await sql`update marketing_native.request_statuses set is_active=false,updated_at=now() where id=${id}::uuid`; return { ok: true, message: "تم تعطيل حالة الطلب" }; }
    const code = clean(body.code).toLowerCase().replace(/[^a-z0-9_]+/g, "_"); const name = clean(body.name);
    if (!code || !name) throw new Error("كود واسم الحالة مطلوبان");
    const [row] = id ? await sql`update marketing_native.request_statuses set code=${code},name=${name},is_terminal=${bool(body.isTerminal)},is_active=${body.isActive !== false},sort_order=${numberValue(body.sortOrder)},updated_at=now() where id=${id}::uuid returning id::text` : await sql`insert into marketing_native.request_statuses(code,name,is_terminal,is_active,sort_order) values(${code},${name},${bool(body.isTerminal)},${body.isActive !== false},${numberValue(body.sortOrder)}) returning id::text`;
    return { ok: true, row, message: "تم حفظ حالة الطلب" };
  }
  if (entity === "attendance_settings") {
    const [row] = await sql`
      update marketing_native.attendance_settings
      set work_start_time=${clean(body.workStartTime) || "16:00"}::time,
          ...
          updated_at=now()
      where ctid=(select ctid from marketing_native.attendance_settings order by updated_at desc nulls last limit 1)
...
async function registerUpload(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  ...
  if (scope === "task") {
    ...
    const [task] = await sql`select id::text,task_kind,assigned_to::text,received_at,status from marketing_native.tasks where id=${entityId}::uuid`;
    if (!task) throw new Error("التاسك غير موجود");
...
async function fileDownload(sql: ReturnType<typeof getSql>, id: string, scope: string, user: SessionUser) {
  ...
  const [row] = scope === "project"
    ? await sql`select pf.id::text,pf.file_name,pf.storage_key,pf.external_url from marketing_native.project_files pf join marketing_native.campaigns c on c.id=pf.campaign_id where pf.id=${id}::uuid and c.is_deleted=false and ${access}`
    : await sql`select up.id::text,up.file_name,up.storage_key,up.external_url from marketing_native.task_uploads up join marketing_native.tasks t on t.id=up.task_id join marketing_native.campaigns c on c.id=t.campaign_id where up.id=${id}::uuid and c.is_deleted=false and ${access}`;
...
async function createRawFolders(sql: ReturnType<typeof getSql>, body: Record<string, unknown>, user: SessionUser) {
  const campaignId = clean(body.campaignId);
  const [project] = await sql`select id::text,campaign_code,name,source_kind from marketing_native.campaigns where id=${campaignId}::uuid and is_deleted=false`;
  if (!project) throw new Error("الحملة أو الأجندة غير موجودة");
  ...
  const creatives = await sql`select instance_no,creative_type,short_code from marketing_native.creatives where campaign_id=${campaignId}::uuid order by sort_order`;
  ...
  await sql`update marketing_native.campaigns set raw_folders_created_at=now(),updated_at=now() where id=${campaignId}::uuid`;
  await sql`insert into marketing_native.activity_log(actor_id,actor_name,action,entity_type,entity_id,details) values(${user.id}::uuid,${user.fullName},'raw_folders_created','project',${campaignId},${sql.json(toDatabaseJson(result))})`;
