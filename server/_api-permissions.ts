...
function marketingRequirement(request: VercelRequest): ApiPermissionRequirement {
  ...
  const map: Record<string, string> = {
    ...
    attach_final_file: "system.marketing.access", prepare_final_upload: "system.marketing.access", commit_final_file_upload: "system.marketing.access", cancel_final_upload: "system.marketing.access", attach_final_media_group: "system.marketing.access",
  ...
  if (route === "owners") {
    ...
    if (settingsScope) {
      return req(
        ...
        "core",
        "settings",
        request.method === "GET" ? "owners_view" : "owners_manage",
      );
    }
    return req(
      request.method === "GET" ? "owners.community.view" : "owners.community.manage",
      "core",
      "owners_community",
