...
-- Marketing Settings is the single source of truth for the marketing department
-- catalog and its users. Each marketing department has one stable central record so
-- the Users & Permissions screen can display the same departments as allowed ones.
alter table marketing.departments
  add column if not exists core_department_id uuid references core.departments(id) on delete restrict;

create table if not exists marketing.integration_state (
  key text primary key,
  completed_at timestamptz not null default now()
);

-- A stable code derived from the marketing department id prevents name changes from
-- creating a second central department.
...
set name=excluded.name,system_code='marketing',is_active=excluded.is_active,updated_at=now();

update marketing.departments md
set core_department_id=cd.id
from core.departments cd
where cd.code='marketing_'||replace(md.id::text,'-','')
  and md.core_department_id is distinct from cd.id;
...
-- One-time preservation of assignments that were previously saved only in the
-- central allowed-departments table. After this migration, Marketing Settings owns
-- membership changes and synchronizes them back to the central table.
insert into marketing.department_users(department_id,user_id)
select md.id,usd.user_id
from marketing.departments md
join core.departments legacy
  on legacy.system_code='marketing'
 and lower(trim(legacy.name))=lower(trim(md.name))
join core.user_system_departments usd
  on usd.department_id=legacy.id and usd.system_code='marketing'
where not exists (
  select 1 from marketing.integration_state state
  where state.key='department_membership_source_v1197'
)
on conflict(department_id,user_id) do nothing;

insert into marketing.integration_state(key)
values('department_membership_source_v1197')
on conflict(key) do nothing;

-- Ensure every selected department user has a system row without silently granting
-- access to marketing. Access remains controlled by Users & Permissions.
insert into core.user_systems(user_id,system_code,is_enabled,role_id,data_scope)
select distinct du.user_id,'marketing',false,null,'workflow_assigned'
from marketing.department_users du
on conflict(user_id,system_code) do nothing;

-- Preserve an old primary selection when legacy allowed departments had one.
insert into core.user_system_departments(user_id,system_code,department_id,is_primary)
select usd.user_id,'marketing',md.core_department_id,bool_or(usd.is_primary)
from marketing.departments md
join core.departments legacy
  on legacy.system_code='marketing'
 and lower(trim(legacy.name))=lower(trim(md.name))
...
where md.core_department_id is not null
  and exists (
    select 1 from marketing.department_users du
    where du.department_id=md.id and du.user_id=usd.user_id
  )
group by usd.user_id,md.core_department_id
...
-- Exact synchronization: only users selected for a department in Marketing Settings
-- remain assigned to its central allowed department.
delete from core.user_system_departments usd
using core.departments cd
where usd.department_id=cd.id
  and usd.system_code='marketing'
  and cd.system_code='marketing'
  and not exists (
    select 1
    from marketing.departments md
    join marketing.department_users du on du.department_id=md.id
    where md.core_department_id=usd.department_id
      and md.is_active=true
      and du.user_id=usd.user_id
  );

insert into core.user_system_departments(user_id,system_code,department_id,is_primary)
select du.user_id,'marketing',md.core_department_id,false
from marketing.department_users du
join marketing.departments md on md.id=du.department_id
where md.core_department_id is not null and md.is_active=true
on conflict(user_id,system_code,department_id) do nothing;

-- Old central marketing departments that do not correspond to the Marketing Settings
-- catalog are hidden, so both screens show the same list without duplicates.
update core.departments cd
set is_active=false,updated_at=now()
where cd.system_code='marketing'
  and not exists (
    select 1 from marketing.departments md
    where md.core_department_id=cd.id and md.is_active=true
  )
  and cd.is_active=true;

-- Every user with marketing departments has one primary department for scope logic.
-- Primary status does not filter the campaign or agenda assignment lists.
with ranked as (
  select usd.user_id,usd.department_id,
         row_number() over(partition by usd.user_id order by usd.is_primary desc,md.is_content desc,md.name,usd.department_id) as row_no
  from core.user_system_departments usd
  join marketing.departments md on md.core_department_id=usd.department_id and md.is_active=true
  where usd.system_code='marketing'
)
update core.user_system_departments usd
set is_primary=(ranked.row_no=1)
from ranked
where usd.user_id=ranked.user_id
  and usd.system_code='marketing'
  and usd.department_id=ranked.department_id
  and usd.is_primary is distinct from (ranked.row_no=1);

-- Keep the legacy aggregate table in sync because older modules still read it.
delete from core.user_departments ud
using core.departments cd
where ud.department_id=cd.id and cd.system_code='marketing';
...
from core.user_system_departments usd
join core.departments cd on cd.id=usd.department_id and cd.system_code='marketing'
where usd.system_code='marketing'
