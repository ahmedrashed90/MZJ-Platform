...
const MARKETING_SCHEMA_VERSION = 2;
...
export async function ensureMarketingSchema() {
  await withDatabaseAdvisoryLock("marketing-schema-v2", async () => {
    const sql = getSql();
    const [row] = await sql<{ version: number | null }[]>`
      select case when to_regclass('marketing.schema_version') is null then null
        else (select max(version)::int from marketing.schema_version) end as version
    `.catch(() => [{ version: null }]);
    if (Number(row?.version || 0) === MARKETING_SCHEMA_VERSION) return;
