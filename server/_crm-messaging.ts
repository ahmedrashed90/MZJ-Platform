...
async function resolveDeliveryPolicy(conversation: ConversationContext): Promise<DeliveryPolicy> {
  ...
  const [sourceConfig] = sourceCode
    ? await sql<any[]>`select name,delivery_route from core.sources where code=${sourceCode} limit 1`
    : [];
  const sourceArabic = clean(sourceConfig?.name)
    || sourceLabel(conversation.source_code || conversation.source_name || conversation.platform_code || conversation.channel_code);
  ...
  if (sourceConfig?.delivery_route) {
    ...
    return {
      ...
      templateOnly: false,
      sourceArabic,
      reason: route === "whatsapp" ? "الإرسال عبر واتساب بنص حر أو قالب" : `الإرسال عبر ${sourceLabel(route)}`,
    };
  }
  ...
  if ((key.includes("facebook") || key.includes("فيسبوك")) && !isLeadSource(`${raw} ${sourceArabic}`)) {
    return { route: "facebook", templateOnly: false, sourceArabic, reason: "الإرسال عبر فيسبوك" };
  }
  if ((key.includes("instagram") || key.includes("انستجرام") || key.includes("انستغرام")) && !isLeadSource(`${raw} ${sourceArabic}`)) {
    return { route: "instagram", templateOnly: false, sourceArabic, reason: "الإرسال عبر إنستجرام" };
  }
  if ((key.includes("tiktok") || key.includes("تيك_توك") || key.includes("تيك")) && !isLeadSource(`${raw} ${sourceArabic}`)) {
    return { route: "tiktok", templateOnly: false, sourceArabic, reason: "الإرسال عبر تيك توك" };
  }

  return { route: "whatsapp", templateOnly: false, sourceArabic, reason: "الإرسال عبر واتساب بنص حر أو قالب" };
}
...
function gatewayHeaders(secretName: string | null | undefined) {
  ...
  return {
    ...
    "x-mzj-gateway-secret": secretValue,
  };
}
...
function extractNumberedTemplateParams(templateBody: string, renderedText: string) {
  ...
  const result = new RegExp(pattern, "u").exec(renderedText);
  if (!result) return [];
...
async function resolveEndpoint(route: DeliveryRoute, kind: DeliveryKind) {
  ...
  const rows = await sql<any[]>`
    ...
    where is_active=true and source_code = any(${candidates})
  ...
  if (!url) {
    throw new Error(`لم يتم ضبط مسار ${kind === "template" ? "القوالب" : kind === "media" ? "الوسائط" : "النص"} لقناة ${sourceLabel(route)}`);
  }
  return { endpoint, url };
}

function responseSucceeded(httpOk: boolean, response: any) {
  const status = clean(response?.status || response?.raw?.status).toLowerCase();
  return httpOk && response?.ok !== false && status !== "error" && status !== "failed";
}

async function postToWorker(url: string, headers: Record<string, string>, payload: Record<string, unknown>): Promise<WorkerResult> {
  try {
    ...
    return { ok: responseSucceeded(provider.ok, response), status: provider.status, response, usedUrl: url };
  } catch (error: any) {
    return { ok: false, status: 0, response: { error: error?.message || String(error) }, usedUrl: url };
  }
}
...
function basePayload(route: DeliveryRoute, conversation: ConversationContext) {
  ...
  if (route === "instagram") {
    return { subscriber_id: participantId, participantId, manychatContactId: participantId, igId: participantId, pageId: conversation.page_id || "", conversationId, convId: conversationId, displayName, serviceKey: conversation.service_key || "" };
  }
...
function deliveryPayload(input: {
  ...
  const payload: Record<string, unknown> = {
    ...basePayload(input.route, input.conversation),
  ...
  if (input.media) {
    ...
    return {
      ...
      attachmentType: input.media.mediaType,
      fileName: input.media.fileName || "",
      mimeType: input.media.mimeType || "",
      fileSize: input.media.fileSize || null,
  ...
  return {
    ...
    message: input.text,
    text: input.text,
    ...(Array.isArray(input.buttons) && input.buttons.length
      ? { buttons: input.buttons, header: input.header || "", footer: input.footer || "" }
      : {}),
  };
}
...
function providerState(delivery: WorkerResult) {
  if (!delivery.ok) return "failed";
  const raw = clean(delivery.response?.status || delivery.response?.raw?.status).toLowerCase();
  if (["sent", "delivered", "read", "queued"].includes(raw)) return raw;
  return "sent";
}

function providerError(delivery: WorkerResult) {
  return clean(
    delivery.response?.error
    || delivery.response?.message
    || delivery.response?.raw?.message
    || delivery.response?.raw,
  ) || (delivery.status ? `HTTP ${delivery.status}` : "تعذر الاتصال بوركر الإرسال");
}
...
export async function deliverCrmMessage(input: {
  ...
  if (job.status !== "queued" && job.processed_at) {
    ...
    return {
      ...
      providerResponse: job.response_payload,
      errorMessage: job.error_message,
      jobId: job.id,
      routing: policy,
      workerRoute: url,
      workerAttempts: [{ url, status: 0, error: null }],
    };
  }

  const delivery = await postToWorker(url, gatewayHeaders(endpoint.secret_name), payload);
  const providerStatus = providerState(delivery);
  const errorMessage = providerStatus === "failed" ? providerError(delivery) : "";
  const workerAttempts = [{ url, status: delivery.status, error: errorMessage || null }];
  ...
  if (!message) {
    [message] = await sql<any[]>`
      insert into crm.messages(
        ...
        is_sensitive,provider_status,sent_by,sender_type,provider_message_id,metadata
      ) values (
        ...
        ${Boolean(input.media?.isSensitive)},${providerStatus},${input.actor?.id || null}::uuid,${senderType},
        ${clean(delivery.response?.message_id || delivery.response?.messageId || delivery.response?.raw?.message_id || delivery.response?.raw?.message_wamid) || null},
        ${sql.json({ jobId: job.id, templateId: input.template?.id || null, reason: input.reason || "manual", deliveryRoute: policy.route, sourceArabic: policy.sourceArabic, workerRoute: url })}
  ...
  await Promise.all([
    sql.unsafe(
      ...
      [finalText || input.media?.fileName || "مرفق", conversation.id, senderType === "human"],
    ),
    sql`
      update integrations.outbound_jobs
      set status=${providerStatus},attempts=attempts+1,response_payload=${sql.json(delivery.response || {})},error_message=${errorMessage || null},processed_at=now()
...
export async function deliverDirectWhatsapp(input: { phone: string; text: string; template?: TemplateRow | null; idempotencyKey?: string; reason?: string }) {
  ...
  const payload = input.template
    ? {
        ...
        components: templateParams.length
          ? [{ type: "body", parameters: templateParams.map((value) => ({ type: "text", text: value })) }]
          : [],
        agentAuto: true,
      }
  ...
  const [job] = await sql<any[]>`
    insert into integrations.outbound_jobs(source,idempotency_key,payload,status)
    values('whatsapp',${key},${sql.json(payload as any)},'queued')
    on conflict(idempotency_key) do update set idempotency_key=excluded.idempotency_key
    returning *,id::text
  `;
  ...
  const delivery = await postToWorker(url, gatewayHeaders(endpoint.secret_name), payload);
  const status = providerState(delivery);
  const error = status === "failed" ? providerError(delivery) : "";
  await sql`
    update integrations.outbound_jobs
    set status=${status},attempts=attempts+1,response_payload=${sql.json(delivery.response || {})},error_message=${error || null},processed_at=now()
    where id=${job.id}::uuid
  `;
  return {
    ok: status !== "failed",
    ...
    workerRoute: url,
    workerAttempts: [{ url, status: delivery.status, error: error || null }],
...
export async function getMappedStatusDraft(input: { leadId: string; departmentCode: string; statusValue: string }) {
  ...
  const [mapping] = await sql<any[]>`
    ...
    where m.department_code=${input.departmentCode} and m.status_value=${input.statusValue} and m.is_active=true and t.is_active=true
    limit 1
  `;
  ...
  const [conversation] = await sql<any[]>`
    ...
    from crm.conversations c join crm.leads l on l.id=c.lead_id
...
export async function sendMappedStatusMessage(input: { leadId: string; departmentCode: string; statusValue: string; actor: SessionUser; eventId?: string | number }) {
  ...
  return {
    skipped: false,
    ...(await deliverCrmMessage({
      conversation,
      text: draft.text,
      template: draft.template,
      actor: input.actor,
      senderType: "human",
      idempotencyKey: input.eventId ? `status-event:${input.eventId}` : undefined,
      reason: "status_change_manual",
    })),
  };
}
