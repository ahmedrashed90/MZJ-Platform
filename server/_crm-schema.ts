...
const CRM_AUTOMATION_FLOW_REBUILD_V1180_SQL = String.raw`
begin;
...
const CRM_KPI_PERMISSIONS_SOLD_QUANTITY_20260728_SQL = String.raw`
begin;
...
const CRM_REPORT_INDICATORS_ALIGNMENT_20260805_SQL = String.raw`
begin;
...
export async function ensureCrmSchema() {
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      const [automationFlowRebuildMigration] = await sql<{ version: string }[]>`
        select version from core.schema_migrations where version = 'crm-automation-flow-rebuild-v1.18.0'
      `;
      if (!automationFlowRebuildMigration) await runSqlScript(CRM_AUTOMATION_FLOW_REBUILD_V1180_SQL);
      const [kpiPermissionsSoldQuantityMigration] = await sql<{ version: string }[]>`
      ...
      const [reportIndicatorsAlignmentMigration] = await sql<{ version: string }[]>`
      ...
      const [salesOrderActualTimeMigration] = await sql<{ version: string }[]>`
