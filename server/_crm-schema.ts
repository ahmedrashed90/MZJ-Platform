...
const CRM_CONVERSATION_AUTOMATION_V1180_SQL = String.raw`
...
export async function ensureCrmSchema() {
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      const [conversationAutomationMigration] = await sql<{ version: string }[]>`
        select version from core.schema_migrations where version = 'crm-conversation-automation-v1.18.0'
      `;
      if (!conversationAutomationMigration) await runSqlScript(CRM_CONVERSATION_AUTOMATION_V1180_SQL);
    })().catch((error) => {
      schemaPromise = null;
      throw error;
    });
