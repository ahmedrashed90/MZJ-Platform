...
create table if not exists operations.locations (
  ...
  code text not null unique,
  name text not null,
  location_type text not null default 'branch' check (location_type in ('branch','warehouse','agency','other')),
...
create table if not exists operations.transfer_requests (
  ...
  department_code text,
  transfer_type text not null default 'transfer' check (transfer_type in ('transfer','photo')),
