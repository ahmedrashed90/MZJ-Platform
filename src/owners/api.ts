...
export async function ownersAdminGet(scope=''){const suffix=scope?`?scope=${encodeURIComponent(scope)}`:'';return read(await fetch(`/api/owners${suffix}`,{credentials:'include',cache:'no-store'}));}
