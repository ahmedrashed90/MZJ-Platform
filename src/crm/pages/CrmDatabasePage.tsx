...
export function CrmDatabasePage() {
  ...
  async function exportRows() {
    ...
    try {
      ...
      downloadXlsx("قاعدة-بيانات-CRM.xlsx", exportRows.map((row) => ({
        ...
        "الدفع": row.payment_type, "نوع التمويل": row.finance_type, "الحالة": row.status_label, "المباع": Number(row.sold_count || 0), "تاريخ تم البيع": formatDate(row.sold_at), "القسم": departmentLabel(row.department_code),
  ...
  async function printRows() {
    ...
    try {
      ...
      popup.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>قاعدة البيانات</title><style>body{font-family:Tajawal,Arial;padding:20px}table{width:100%;border-collapse:collapse;font-size:11px}th,td{border:1px solid #ddd;padding:6px;text-align:right}th{background:#f5e8df}</style></head><body><h1>قاعدة البيانات</h1><p>عدد السجلات: ${printRows.length}</p><table><thead><tr><th>اسم العميل</th><th>الجوال</th><th>الفرع</th><th>المصدر</th><th>السيارة</th><th>الحالة</th><th>المباع</th><th>تاريخ تم البيع</th><th>القسم</th><th>المسؤول</th><th>الكول سنتر</th><th>آخر تحديث</th></tr></thead><tbody>${printRows.map((row) => `<tr><td>${escapeHtml(row.customer_name)}</td><td>${escapeHtml(row.phone || row.phone_normalized)}</td><td>${escapeHtml(row.branch_name || row.branch_code)}</td><td>${escapeHtml(sourceLabel(row.source_code, row.source_name))}</td><td>${escapeHtml(row.car_name)}</td><td>${escapeHtml(row.status_label)}</td><td>${escapeHtml(Number(row.sold_count || 0))}</td><td>${escapeHtml(formatDate(row.sold_at))}</td><td>${escapeHtml(departmentLabel(row.department_code))}</td><td>${escapeHtml(row.assigned_name)}</td><td>${escapeHtml(row.call_center_name)}</td><td>${escapeHtml(formatDate(row.updated_at))}</td></tr>`).join("")}</tbody></table><script>window.onload=()=>window.print()</script></body></html>`);
  ...
  return (
    <div className="crm-page crm-database-page">
      ...
      <div className="crm-filter-panel">
        ...
        <select value={filters.department} onChange={(event) => setFilter("department", event.target.value)}><option value="">كل الأقسام</option><option value="cash_sales">مبيعات الكاش</option><option value="finance_sales">مبيعات التمويل</option><option value="wholesale">قسم الجملة</option><option value="customer_service">خدمة العملاء</option><option value="call_center">كول سنتر</option></select>
      ...
      <LeadDrawer mode="edit" lead={selected} meta={meta} onClose={() => setSelected(null)} onRead={(updated) => { setRows((current) => current.map((row) => row.id === updated.id ? { ...row, ...updated } : row)); setSelected((current) => current?.id === updated.id ? { ...current, ...updated } : current); }} onSaved={(updated) => { setRows((current) => current.map((row) => row.id === updated.id ? { ...row, ...updated } : row)); setSelected(null); }} />
