...
import { ArrowClockwise, CheckCircle, FloppyDisk, Play, Robot, Trash, XCircle } from "@phosphor-icons/react";
...
const defaults = {
  ...
  socialEnabled: false,
  socialWorkerUrl: "",
  socialConversationLimit: 50,
  socialMessageLimit: 30,
...
function fromApi(raw: any) {
  ...
  return {
    ...
    socialEnabled: Boolean(raw.social_enabled), socialWorkerUrl: raw.social_worker_url || "", socialConversationLimit: raw.social_conversation_limit ?? 50,
    socialMessageLimit: raw.social_message_limit ?? 30, socialPlatforms: raw.social_platforms || defaults.socialPlatforms,
  };
}
...
export function CrmInboxAgentPage() {
  ...
  async function testSocial() {
    if (!settings.socialWorkerUrl) { setNotice("أضف رابط Worker المنصات الاجتماعية أولاً."); return; }
    try {
      const response = await fetch(settings.socialWorkerUrl, { method: "GET" });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      setNotice("تم تشغيل اختبار وكيل المنصات الاجتماعية.");
    } catch (error) { setNotice(`فشل تشغيل الاختبار: ${error instanceof Error ? error.message : error}`); }
  }

  return (
    <div className="crm-page inbox-agent-page">
      <header className="crm-page-head"><div><h1>وكيل صندوق الوارد</h1><p>إعداد ومراقبة وكيل واتساب ووكيل المنصات الاجتماعية. واتساب يرد ويصعّد، أما إنستجرام / فيسبوك / تيك توك فهي مراقبة وتصعيد فقط بدون الرد على العميل.</p></div><button className="crm-secondary-button" onClick={() => void load()}><ArrowClockwise size={18} />تحديث</button></header>
      ...
      {tab === "general" ? <div className="crm-settings-grid">
        ...
        <section className="crm-panel settings-card"><h2>وكيل المنصات الاجتماعية</h2><label className="crm-switch-row"><input type="checkbox" checked={settings.socialEnabled} onChange={(event) => set("socialEnabled", event.target.checked)} /><span>تفعيل وكيل المنصات الاجتماعية</span></label><label className="crm-form-label"><span>رابط Social Worker</span><input value={settings.socialWorkerUrl} onChange={(e) => set("socialWorkerUrl", e.target.value)} /></label><div className="crm-form-grid"><label><span>حد فحص المحادثات</span><input type="number" value={settings.socialConversationLimit} onChange={(e) => set("socialConversationLimit", Number(e.target.value))} /></label><label><span>حد قراءة الرسائل لكل محادثة</span><input type="number" value={settings.socialMessageLimit} onChange={(e) => set("socialMessageLimit", Number(e.target.value))} /></label></div><p className="crm-help-text">هذا الوكيل لا يرد على العميل في إنستجرام أو فيسبوك أو تيك توك. يراقب آخر رسالة عميل وآخر رد CRM ثم يصعّد عبر واتساب عند التأخير.</p><button className="crm-secondary-button" onClick={() => void testSocial()}><Play size={17} />تشغيل اختبار</button></section>
        <section className="crm-panel settings-card full"><h2>مديرو التصعيد</h2><div className="crm-manager-form"><select value={manager.scopeCode} onChange={(e) => setManager((current) => ({ ...current, scopeCode: e.target.value }))}><option value="">القسم / الفرع</option><option value="cash_sales">مبيعات الكاش</option><option value="finance_sales">مبيعات التمويل</option><option value="customer_service">خدمة العملاء</option><option value="call_center">الكول سنتر</option></select><input placeholder="اسم المدير" value={manager.managerName} onChange={(e) => setManager((current) => ({ ...current, managerName: e.target.value }))} /><input placeholder="رقم واتساب المدير" value={manager.whatsappPhone} onChange={(e) => setManager((current) => ({ ...current, whatsappPhone: e.target.value }))} /><button className="crm-primary-button" onClick={() => void saveManager()}>إضافة / تحديث</button></div><div className="crm-table-shell compact"><table className="crm-table"><thead><tr><th>الفرع / القسم</th><th>اسم المدير</th><th>رقم الواتساب</th><th>الحالة</th><th>الإجراءات</th></tr></thead><tbody>{managers.map((row) => <tr key={row.id}><td>{row.scope_code}</td><td>{row.manager_name}</td><td>{row.whatsapp_phone}</td><td>{row.is_active ? "مفعل" : "متوقف"}</td><td><button className="crm-row-delete" onClick={() => void deleteManager(row.scope_code)}><Trash size={16} />مسح</button></td></tr>)}{!managers.length ? <tr><td colSpan={5}><div className="crm-empty-state">لا توجد إعدادات مديرين بعد.</div></td></tr> : null}</tbody></table></div></section>
