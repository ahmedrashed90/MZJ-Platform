...
export function CrmInboxAgentPage() {
  const { user } = useAuth();
  const canManage = hasPermission(user, "crm.inbox_agent.manage");
  const [tab, setTab] = useState<"general" | "replies" | "logs">(canManage ? "general" : "logs");
  ...
  return (
    <div className="crm-page inbox-agent-page">
      ...
      <div className="crm-department-tabs">{canManage ? <><button className={tab === "general" ? "active" : ""} onClick={() => setTab("general")}>الإعدادات العامة</button><button className={tab === "replies" ? "active" : ""} onClick={() => setTab("replies")}>الردود</button></> : null}<button className={tab === "logs" ? "active" : ""} onClick={() => setTab("logs")}>السجل</button></div>

      {tab === "general" && canManage ? <div className="crm-settings-grid">
