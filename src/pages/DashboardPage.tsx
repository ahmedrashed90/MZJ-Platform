...
function DetailsDrawer({ details, onClose, onLeadOpen }: { details: DetailPayload | null; onClose: () => void; onLeadOpen: (item: DashboardLeadItem) => void }) {
  ...
  const exportOperations = () => {
    if (details.operationVehicles) {
      ...
      downloadExcel(details.title, ["رقم الطلب", "منشئ الطلب", "تاريخ الطلب", "الحالة", "VIN", "المصدر", "الوجهة"], requests.map((row) => [row.request_no, row.requested_by_name, formatOperationsDate(row.requested_at), requestStatusLabel(row.status), row.vins, row.source_location_name, row.destination_location_name]));
...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const loadDashboard = () => {
      fetch("/api/dashboard", { cache: "no-store" })
        ...
        .then((payload: DashboardData) => { if (active) setData(payload); })
        .catch(() => { if (active) setData(null); })
        .finally(() => { if (active) setLoading(false); });
    };
    loadDashboard();
    window.addEventListener("operations:data-changed", loadDashboard);
    return () => { active = false; window.removeEventListener("operations:data-changed", loadDashboard); };
  }, []);
  ...
  async function openOperationRequests(title: string, status = "", type = "") {
    const requestId = ++detailsRequestId.current;
    setDetails({ title, subtitle: "نفس بيانات تبويب طلبات النقل", loading: true, operationRequests: [] });
    try {
      ...
      <div className="dashboard-page">
        ...
        <section className="operations-dashboard-section">
          ...
          <div className="operations-grid lower-row">
            ...
            <OperationCard title="طلبات النقل" badge={operations?.transfers.total ?? null} onView={() => void openOperationRequests("طلبات النقل")}>
              <div className="operation-metrics-grid">
