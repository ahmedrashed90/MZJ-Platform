...
export function TransferRequestsPage() {
  ...
  const destination = useMemo(() => meta.locations.find((item) => item.id === destinationLocationId), [destinationLocationId, meta.locations]);
  ...
  useEffect(() => {
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        setResults(payload.rows.filter((row) => !selectedCars.some((item) => item.id === row.id) && !row.active_transfer_requests));
      } catch (failure) {
  ...
  async function create() {
    ...
    try {
      const payload = await operationsFetch<{ message: string }>("/api/operations", {
        method: "POST",
        body: JSON.stringify({ action: "create_transfer", vehicleIds: selectedCars.map((item) => item.id), destinationLocationId, note }),
      });
  ...
  async function stageAction(row: TransferRow, target = nextStage[row.status]) {
    ...
    setMessage("");
    try {
      const payload = await operationsFetch<{ message: string }>("/api/operations", { method: "POST", body: JSON.stringify({ action: "transfer_action", id: row.id, transferAction: "advance", nextStatus: target }) });
      setMessage(payload.message);
      setSelected(null);
      await loadRows();
    } catch (failure) {
  ...
  async function destructiveAction() {
    ...
    setError("");
    try {
      const payload = await operationsFetch<{ message: string }>("/api/operations", { method: "POST", body: JSON.stringify({ action: "transfer_action", id: selected.id, transferAction: confirmAction, reason }) });
      setMessage(payload.message);
      ...
      {tab === "create" ? (
        <section className="panel operations-transfer-create">
          <div className="operations-transfer-controls">
            <OperationsVehiclePicker search={search} results={results} placeholder="ابحث برقم الهيكل أو السيارة أو البيان" onSearchChange={setSearch} onSelect={addCar} />
            <label className="operations-control-field"><span>المكان المستهدف</span><select value={destinationLocationId} onChange={(event) => setDestinationLocationId(event.target.value)}><option value="">اختر المكان</option>{meta.locations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          </div>

          {!selectedCars.length ? (
            <div className="operations-empty-state"><Truck size={42} weight="duotone" /><strong>لم يتم اختيار سيارات</strong><span>السيارات المرتبطة بطلب نقل نشط لن تظهر ضمن نتائج البحث.</span></div>
          ) : (
            <div className="operations-selection-table-wrap">
              <div className="operations-selection-summary"><strong>{selectedCars.length.toLocaleString("ar-SA")} سيارة داخل الطلب</strong><span>{destination ? <>المكان المستهدف: <b>{destination.name}</b></> : "حدد المكان المستهدف"}</span></div>
              <ResizableOperationsTable<VehicleRow>
                rows={selectedCars}
                columns={createColumns}
                rowKey={(row) => row.id}
                storageKey="mzj.operations.transferCreate.columnWidths.v1"
          ...
          <div className="operations-transfer-detail">
            ...
            <div className="operations-request-vehicles-table-wrap">
              ...
              <ResizableOperationsTable<TransferVehicle>
                ...
                storageKey="mzj.operations.transferDetails.columnWidths.v1"
