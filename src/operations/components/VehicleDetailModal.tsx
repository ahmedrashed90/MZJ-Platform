      ...
      <Modal open={Boolean(id)} title={vehicle ? `${vehicle.vin} — ${vehicle.car_name || "سيارة"}` : "تفاصيل السيارة"} subtitle={vehicle ? `${vehicle.location_name || "—"} · ${vehicle.status_name || "—"}` : undefined} onClose={onClose} className="operations-detail-modal">
        ...
        {loading || !vehicle ? <div className="operations-loading">جاري تحميل ملف السيارة...</div> : (
          <>
            ...
            {tab === "tracking" ? <div className="operations-timeline">{vehicle.tracking.length ? vehicle.tracking.map((row) => <article key={row.id}><LinkSimple size={18} /><div><strong>{row.sales_order_no}</strong><span>{row.status} — {row.progress}%</span><small>{formatOperationsDate(row.updated_at)}</small></div></article>) : <p>لا يوجد طلب تراكينج مرتبط.</p>}</div> : null}
