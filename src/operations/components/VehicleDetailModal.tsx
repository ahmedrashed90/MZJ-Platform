...
export function VehicleDetailModal({ id, meta, onClose, onChanged }: { id: string | null; meta: OperationsMeta; onClose: () => void; onChanged?: () => void }) {
  ...
  async function runAction() {
    if (!vehicle || !confirmAction || !reason.trim()) return;
    setActionLoading(true); setError("");
    try {
      await operationsFetch("/api/operations", { method: "POST", body: JSON.stringify({ action: confirmAction === "delete" ? "delete_vehicle" : "archive_vehicle", id: vehicle.id, reason }) });
      setConfirmAction(null); setReason(""); onChanged?.(); onClose();
  ...
  return (
    <>
      <Modal open={Boolean(id)} title={vehicle ? `${vehicle.vin} — ${vehicle.car_name || "سيارة"}` : "تفاصيل السيارة"} subtitle={vehicle ? `${vehicle.location_name || "—"} · ${vehicle.status_name || "—"}` : undefined} onClose={onClose} className="operations-detail-modal">
        ...
        {loading || !vehicle ? <div className="operations-loading">جاري تحميل ملف السيارة...</div> : (
          <>
            ...
            {tab === "checks" ? <div className="operations-check-grid">{vehicle.checks.map((item) => <div key={item.code}><span>{item.name}</span><b data-status={item.status}>{item.status === "ok" ? "موجود" : item.status === "missing" ? "ناقص" : "غير محدد"}</b><small>{item.note || ""}</small></div>)}</div> : null}
            ...
            {tab === "tracking" ? <div className="operations-timeline operations-detail-tracking-list">{vehicle.tracking.length ? vehicle.tracking.map((row) => <button type="button" key={row.id} className="operations-tracking-detail-button" onClick={() => window.location.assign(`/tracking?order=${encodeURIComponent(row.id || "")}`)}><LinkSimple size={20} /><div><strong>{row.sales_order_no}</strong><span>{row.status} — {row.progress}%</span><i><span style={{ width: `${Math.max(0, Math.min(100, Number(row.progress || 0)))}%` }} /></i><small>{formatOperationsDate(row.updated_at)}</small></div></button>) : <p>لا يوجد طلب تراكينج مرتبط.</p>}</div> : null}
      ...
      <Modal open={Boolean(confirmAction)} title={confirmAction === "delete" ? "تأكيد مسح السيارة" : "تأكيد أرشفة السيارة"} subtitle={vehicle ? `${vehicle.vin} — ${vehicle.car_name || "سيارة"}` : undefined} onClose={() => { if (!actionLoading) { setConfirmAction(null); setReason(""); } }} className="operations-confirm-modal" level={1} initialFocusRef={reasonRef} footer={<><button type="button" className="secondary" onClick={() => setConfirmAction(null)} disabled={actionLoading}>إلغاء</button><button type="button" className={confirmAction === "delete" ? "danger" : "primary"} onClick={() => void runAction()} disabled={actionLoading || !reason.trim()}>{actionLoading ? "جاري التنفيذ..." : "تأكيد نهائي"}</button></>}>
        <div className={`operations-confirm-warning ${confirmAction === "delete" ? "danger" : ""}`}><WarningCircle size={24} /><p>{confirmAction === "delete" ? "هذا حذف فيزيائي نهائي، ولن يُسمح به إذا كان للسيارة أي تاريخ تشغيلي." : "ستخرج السيارة من المخزون النشط مع الحفاظ على كل تاريخها."}</p></div>
