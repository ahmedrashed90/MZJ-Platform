...
const columns: Column[] = [
  ...
  { key: "tracking", label: "Tracking", width: 175, min: 130, max: 300, value: (row) => row.tracking_order_no, render: (row) => row.tracking_order_id ? <button type="button" className="operations-inline-link" onClick={() => window.location.assign(`/tracking?order=${encodeURIComponent(row.tracking_order_id || "")}`)}>{row.tracking_order_no || "طلب"} — {row.tracking_progress || 0}%</button> : <span className="operations-muted-badge">لا يوجد طلب</span> },
