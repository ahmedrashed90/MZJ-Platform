...
export function DashboardOperationsModal({ selection, onClose }: { selection: DashboardOperationsSelection | null; onClose: () => void }) {
  ...
  return (
    <>
            ...
            {!loading && !requestRows.length ? <div className="operations-empty-state">لا توجد طلبات</div> : requestRows.map((row) => (
              <article key={row.id}>
                <div>
                  ...
                  <small>{row.cancelled_at ? "ملغي" : requestStatusLabels[row.status || ""] || row.status || "—"} · {row.requested_by_name || row.creator_name || "—"} · {row.requested_at ? new Date(row.requested_at).toLocaleString("ar-SA") : "—"}{row.request_kind === "photography" ? ` · تاريخ التصوير: ${row.photography_date ? new Date(row.photography_date).toLocaleDateString("ar-SA") : "—"}` : ""}</small>
                </div>
      ...
      <Modal open={Boolean(detail)} level={1} title={`تفاصيل ${detail?.request_no || "الطلب"}`} subtitle={detail?.request_kind === "photography" ? `تاريخ التصوير: ${detail.photography_date ? new Date(detail.photography_date).toLocaleDateString("ar-SA") : "—"}` : undefined} onClose={() => setDetail(null)} className="dashboard-request-detail-modal">
