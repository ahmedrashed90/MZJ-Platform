...
import { ArrowLeft, ArrowRight, CalendarBlank, CheckCircle, CurrencyCircleDollar, FloppyDisk, PencilSimple, Plus, PlusCircle, Trash, X } from "@phosphor-icons/react";
...
export function EntityCreativeManager({
  ...
  return <Modal
    ...
    level={2}
    footer={<div className="marketing-entity-creative-footer-bar">
        ...
        <section className="marketing-entity-creative-body">
          ...
          {sourceType === "campaign" && step === 1 ? <div className="marketing-budget-list marketing-campaign-budget-step">
            ...
            {budgets.length ? budgets.map((budget, index) => <article key={budget.id} className="marketing-budget-card">
              ...
              <footer className="marketing-budget-card-total"><span>إجمالي البند</span><strong>{budget.platformAmounts.reduce((sum, item) => sum + Number(item.amount || 0), 0).toLocaleString("ar-SA-u-nu-latn")} ر.س</strong></footer>
            ...
            <div className="marketing-total-budget"><span>إجمالي ميزانية الكرييتيف</span><strong>{totalBudget.toLocaleString("ar-SA-u-nu-latn")} ر.س</strong></div>
          </div> : null}
