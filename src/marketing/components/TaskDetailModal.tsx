    ...
    <Modal open={Boolean(taskId)} title={task?.title || "تفاصيل التاسك"} subtitle={task ? `${task.source_name || "—"} · ${task.department_name || "قسم المحتوى"}` : undefined} onClose={onClose} className="marketing-task-modal">
      ...
      {task ? <div className="marketing-task-detail">
        ...
        {task.task_kind === "task_template" ? <>
          <section className="marketing-task-section"><h3>Task Template</h3><p>{task.template_department_note || task.note || "لا توجد ملاحظات"}</p><div className="marketing-inline-actions"><button type="button" className="secondary" onClick={() => downloadTaskTemplate(task)}><DownloadSimple size={17} />تحميل Task Template</button><label className="marketing-upload-button"><FileArrowUp size={17} />إرفاق Task Template Excel<input type="file" accept=".xls" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadTemplate(file); event.currentTarget.value = ""; }} /></label>{task.template_file_id ? <button type="button" className="secondary" onClick={() => void downloadMarketingFile(task.template_file_id)}><DownloadSimple size={17} />تحميل الملف المرفوع</button> : null}</div></section>
          <section className="marketing-task-section"><h3>بيانات كاتب المحتوى</h3><div className="marketing-form-grid">{Object.entries(writerLabels).map(([key, label]) => <label key={key}><span>{label}</span><textarea rows={key === "mainScript" ? 4 : 2} value={editData[key] || ""} disabled={!payload.isAdmin} onChange={(event) => setEditData((current) => ({ ...current, [key]: event.target.value }))} /></label>)}</div></section>
          {payload.isAdmin ? <section className="marketing-task-section admin"><h3><ShieldCheck size={21} />مراجعة واعتماد</h3><label><span>ملاحظة مدير النظام</span><textarea rows={3} value={adminNote} onChange={(event) => setAdminNote(event.target.value)} /></label><div className="marketing-review-actions"><button type="button" onClick={() => void action({ action: "review_template", templateId: task.task_template_id, reviewAction: "request_edit", note: adminNote, data: editData })}>طلب تعديل</button><button type="button" onClick={() => void action({ action: "review_template", templateId: task.task_template_id, reviewAction: "edit", note: adminNote, data: editData })}>تعديل</button><button type="button" className="primary" onClick={() => void action({ action: "review_template", templateId: task.task_template_id, reviewAction: "approve", note: adminNote, data: editData })}><CheckCircle size={17} />اعتماد التعليمات</button><button type="button" className="danger" onClick={() => void action({ action: "review_template", templateId: task.task_template_id, reviewAction: "reject", note: adminNote, data: editData })}>مرفوض</button></div></section> : null}
