  ...
  const content = <>
    ...
    <div className="marketing-settings-grid">
      <section className="marketing-card">
        ...
        <p className="marketing-picker-empty">تحديد يوزرات القسم يتم من الإعدادات ← المستخدمون والصلاحيات ← المستخدمون ← التسويق ← الأقسام المسموحة.</p>
      ...
      <section className="marketing-card marketing-list-card">
        <h2>قائمة الأقسام</h2>
        {(meta?.departments || []).map((item) => <article key={item.id}><div><strong>{item.name}</strong><small>{item.is_content ? "قسم محتوى" : "قسم تنفيذي"} · تعيين المستخدمين من المستخدمون والصلاحيات</small></div><div className="marketing-inline-actions"><button onClick={() => setDepartment({ id: item.id, name: item.name, isContent: item.is_content })}><PencilSimple /></button><button className="danger" onClick={() => void remove("department", item.id)}><Trash /></button></div></article>)}
      </section>
