  ...
  const content = <>
    ...
    <div className="marketing-settings-grid">
      <section className="marketing-card">
        ...
        <label>اليوزرات داخل القسم<select multiple value={department.userIds} onChange={(e) => setDepartment({ ...department, userIds: Array.from(e.target.selectedOptions).map((option) => option.value) })}>{(meta?.allUsers || meta?.users || []).map((user) => <option key={user.id} value={user.id}>{user.full_name || user.fullName}{user.email ? ` — ${user.email}` : ""}</option>)}</select></label>
