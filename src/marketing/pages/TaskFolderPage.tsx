...
export function TaskFolderPage() {
  ...
  function downloadFolder(targetFolderId?: string) {
    const id = String(targetFolderId || payload?.currentFolderId || "").trim();
    if (!id) return;
    const url = `/api/marketing${marketingQuery({ resource: "task_folder_archive", taskId, kind, folderId: id })}`;
    window.open(url, "_blank", "noopener,noreferrer");
  }
  ...
  async function uploadFiles(files: FileList | null, mode: "files" | "folder" = "files") {
    ...
    try {
      for (let index = 0; index < selected.length; index += 1) {
        ...
        const relativePath = mode === "folder" ? String(file.webkitRelativePath || "").trim() : "";
        const displayName = relativePath || file.name;
        setUpload({ fileName: displayName, fileIndex: index, fileCount: selected.length, loaded: 0, total: file.size, percent: 0, speed: 0, eta: null, status: "uploading" });
        const prepared = await marketingFetch<{ uploadUrl: string; verificationKey: string; folderId: string }>("/api/marketing", {
          method: "POST",
          body: JSON.stringify({
      ...
      if (mode === "folder") {
        const rootFolderName = String(selected[0]?.webkitRelativePath || "").split("/").filter(Boolean)[0] || "الفولدر";
        setMessage(`تم رفع فولدر "${rootFolderName}" بالكامل`);
      } else {
  ...
  return (
    <div className="marketing-task-folder-page">
      <div className="marketing-task-folder-head">
        ...
        <div className="marketing-task-folder-head-actions">
          {payload ? <button type="button" className="marketing-task-folder-download secondary" onClick={() => downloadFolder(payload.currentFolderId)}>
            <DownloadSimple size={18} />تحميل الفولدر
          </button> : null}
          {payload?.permissions.canUpload ? <label className="marketing-task-folder-upload secondary">
            <FolderOpen size={18} />رفع فولدر
            <input
              ref={folderInputRef}
              type="file"
              multiple
              {...({ webkitdirectory: "", directory: "" } as any)}
              onChange={(event) => void uploadFiles(event.target.files, "folder")}
            />
          </label> : null}
          {payload?.permissions.canUpload ? <label className="marketing-task-folder-upload secondary">
            <UploadSimple size={18} />رفع ملفات
            <input ref={fileInputRef} type="file" multiple onChange={(event) => void uploadFiles(event.target.files, "files")} />
          </label> : null}
        </div>
      </div>
        ...
        {payload?.items.length ? <div className="marketing-task-folder-list">
          {payload.items.map((item) => (
            <article key={item.id} className={`marketing-task-folder-row ${item.isFolder ? "folder" : "file"}`}>
              ...
              <div className="marketing-task-folder-actions">
                <button
                  type="button"
                  className="secondary ghost"
                  title={item.isFolder ? "تحميل الفولدر كامل" : "تحميل"}
                  onClick={() => item.isFolder ? downloadFolder(item.id) : downloadFile(item)}
                >
                  <DownloadSimple size={18} />
                </button>
                {!item.isFolder && item.canDelete ? <button type="button" className="danger ghost" title="مسح" onClick={() => void deleteFile(item)}><Trash size={18} /></button> : null}
              </div>
            </article>
          ))}
        </div> : null}
