  ...
  return <MarketingPage title="قاعدة البيانات" description="الحملات والأجندات وملفات النتائج وروابط الحملة والأرشفة." actions={<input className="marketing-search" placeholder="بحث" value={search} onChange={(event) => setSearch(event.target.value)} />}>
    ...
    <Modal open={Boolean(selected)} title={selected ? `عرض البيانات — ${selected.name}` : "عرض البيانات"} subtitle={selected?.code || undefined} onClose={() => { setSelected(null); setDetail(null); }} className="marketing-database-modal">
      ...
      {detail ? <div className="marketing-entity-detail print-area">
        ...
        <section className="marketing-task-section" id="marketing-product-files"><h3>عرض ملفات المنتجات</h3><div className="marketing-files-list">{canDownloadFiles ? detail.files.filter((file: any) => file.category === "final-file").map((file: any) => <button key={file.id} type="button" onClick={() => void downloadMarketingFile(file.id)}><DownloadSimple size={17} />{file.original_name}</button>) : null}{!detail.files.some((file: any) => file.category === "final-file") ? <p>لا توجد ملفات نهائية مرفوعة.</p> : null}</div></section>
        ...
        <section className="marketing-task-section"><h3>عرض نتائج الحملة</h3><div className="marketing-inline-actions">{canUploadResults ? <label className="marketing-upload-button"><FileArrowUp size={17} />رفع ملف النتائج<input type="file" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadResult(file); event.currentTarget.value = ""; }} /></label> : null}{detail.entity.result_file_id && canDownloadFiles ? <button type="button" className="secondary" onClick={() => void downloadMarketingFile(detail.entity.result_file_id)}><DownloadSimple size={17} />عرض الملف المرفوع</button> : null}</div></section>

        <section className="marketing-task-section"><h3>روابط الحملة</h3>{links.map((link, index) => <div className="marketing-link-row" key={index}><select value={link.platform} disabled={!canEditLinks} onChange={(event) => setLinks((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, platform: event.target.value } : item))}><option value="">اختر المنصة</option><option value="Facebook">Facebook</option><option value="Instagram">Instagram</option></select><input dir="ltr" placeholder="https://" disabled={!canEditLinks} value={link.url} onChange={(event) => setLinks((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, url: event.target.value } : item))} />{canEditLinks ? <button type="button" className="icon-danger" onClick={() => setLinks((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash size={16} /></button> : null}</div>)}{canEditLinks ? <div className="marketing-inline-actions"><button type="button" className="secondary" onClick={() => setLinks((current) => [...current, { platform: "", url: "" }])}><LinkSimple size={17} />إضافة منصة ورابط</button><button type="button" className="primary" onClick={() => void saveLinks()}>حفظ الروابط</button></div> : null}</section>

        {canArchive ? <section className="marketing-task-section warning"><h3><WarningCircle size={20} />الأرشفة</h3><p>لا يمكن أرشفة الحملة قبل رفع ملف نتائج الحملة وإضافة روابط الحملة.</p><button type="button" className="secondary" onClick={() => void action("archive_entity", selected)}><Archive size={17} />أرشيف</button></section> : null}
      </div> : null}
    </Modal>
  </MarketingPage>;
}
