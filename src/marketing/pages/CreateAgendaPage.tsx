...
import { CalendarPlus, CheckCircle, FolderOpen, PencilSimple, Plus } from "@phosphor-icons/react";
...
export function CreateAgendaPage() {
  ...
  const [newCreativeTypeId, setNewCreativeTypeId] = useState("");
  const [newCreativeQuantity, setNewCreativeQuantity] = useState(1);
  ...
  function openDay(date: string) {
    setNewCreativeTypeId("");
    setNewCreativeQuantity(1);
    setEditingDay(date);
  }
  ...
  function addCreativeToSelectedDay() {
    if (!editingDay || !newCreativeTypeId) {
      setError("اختر نوع الكرييتيف أولًا");
      return;
    }
    const day = days.find((item) => item.date === editingDay);
    if (!day) return;
    const draft = newCreativeDraft();
    draft.creativeTypeId = newCreativeTypeId;
    draft.quantity = Math.max(1, newCreativeQuantity || 1);
    updateDay(editingDay, [...day.creatives, draft]);
    setNewCreativeTypeId("");
    setNewCreativeQuantity(1);
    setError("");
  }
  ...
  async function downloadZip() {
    const JSZip = await getJSZip(); const zip = new JSZip(); zip.file("agenda-relationships.csv", relationshipCsv(relations));
    const blob = await zip.generateAsync({ type: "blob" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `${form.monthKey || "agenda"}-relationships.zip`; anchor.click(); URL.revokeObjectURL(url);
  }
  ...
  return <MarketingPage title="إنشاء أجندة" description="إنشاء الأجندة وربط كرييتيفات كل يوم بالأقسام واليوزرات والمنصات.">
    <div className="marketing-wizard-steps three">{["بيانات الأجندة", "جدول الأيام والربط", "مراجعة وإنشاء الأجندة"].map((label, index) => <button key={label} type="button" className={step === index ? "active" : index < step ? "done" : ""} onClick={() => index <= step && setStep(index)}><span>{index + 1}</span><b>{label}</b></button>)}</div>
    ...
    <section className="panel marketing-wizard-panel marketing-agenda-wizard">
      {step === 0 ? <div className="marketing-form-grid"><label><span>الشهر</span><input type="month" value={form.monthKey} onChange={(event) => setForm({ ...form, monthKey: event.target.value })} /></label><label><span>اسم الأجندة</span><input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label><span>بداية النشر</span><input type="date" value={form.publishStart} onChange={(event) => setForm({ ...form, publishStart: event.target.value })} /></label><label><span>نهاية النشر</span><input type="date" min={form.publishStart} value={form.publishEnd} onChange={(event) => setForm({ ...form, publishEnd: event.target.value })} /></label><div className="full marketing-inline-actions"><button type="button" className="primary" onClick={prepareDays}><CalendarPlus size={17} />التالي</button></div></div> : null}

      {step === 1 ? <div className="marketing-agenda-days-view"><header className="marketing-editor-page-head"><div><h2>جدول الأيام والربط</h2><p>أضف كرييتيفات كل يوم واربط الأقسام واليوزرات والسيارات والمنصات.</p></div><CalendarPlus size={24} weight="duotone" /></header><div className="marketing-agenda-day-list">{days.map((day) => <article key={day.date}><div className="marketing-agenda-day-date"><strong>{new Date(`${day.date}T00:00:00Z`).toLocaleDateString("ar-SA", { weekday: "long" })}</strong><span>{new Date(`${day.date}T00:00:00Z`).toLocaleDateString("ar-SA", { year: "numeric", month: "long", day: "numeric" })}</span></div><div className="marketing-agenda-day-summary"><strong>{day.creatives.length ? day.creatives.map((creative) => creativeName(creative.creativeTypeId)).join("، ") : "لا توجد كرييتيفات"}</strong><span>{day.creatives.reduce((sum, item) => sum + item.quantity, 0).toLocaleString("ar-SA")} كرييتيف</span></div><button type="button" className="secondary" onClick={() => openDay(day.date)}><PencilSimple size={16} />إضافة / تعديل الربط</button></article>)}</div><div className="marketing-wizard-footer"><span /><button type="button" className="primary" onClick={() => setStep(2)}>التالي: المراجعة والإنشاء</button></div></div> : null}

      {step === 2 ? <div className="marketing-review"><div className="marketing-review-grid"><article><small>الشهر</small><strong>{form.monthKey}</strong></article><article><small>اسم الأجندة</small><strong>{form.name}</strong></article><article><small>الأيام</small><strong>{days.length}</strong></article><article><small>الأيام المستخدمة</small><strong>{days.filter((day) => day.creatives.length).length}</strong></article><article><small>الكرييتيفات</small><strong>{totalCreatives}</strong></article><article><small>العلاقات</small><strong>{relations.length}</strong></article><article><small>Task Templates</small><strong>{totalTemplates}</strong></article><article><small>إجمالي التاسكات</small><strong>{totalTemplates + relations.length}</strong></article></div><div className="marketing-review-table"><table><thead><tr><th>اليوم</th><th>الكرييتيف</th><th>العدد</th><th>المنصات وأنواع النشر</th></tr></thead><tbody>{days.flatMap((day) => day.creatives.map((creative) => <tr key={`${day.date}-${creative.tempId}`}><td>{day.date}</td><td>{creativeName(creative.creativeTypeId)}</td><td>{creative.quantity}</td><td>{creative.platforms.map((platform) => { const name = meta.platforms.find((item) => item.id === platform.platformId)?.name; const types = platform.postTypeIds.map((id) => meta.postTypes.find((item) => item.id === id)?.name).filter(Boolean).join("، "); return `${name}: ${types}`; }).join(" | ") || "—"}</td></tr>))}</tbody></table></div><div className="marketing-inline-actions"><button type="button" className="secondary" onClick={() => void rawFolders()} disabled={loading}><FolderOpen size={17} />إنشاء فولدرات الخام</button><button type="button" className="secondary" onClick={() => void downloadZip()}>تحميل شيتات العلاقات ZIP</button><button type="button" className="primary" onClick={() => void create()} disabled={loading}><CheckCircle size={17} />{loading ? "جاري إنشاء الأجندة..." : "إنشاء الأجندة"}</button></div></div> : null}

      {step > 0 ? <footer className="marketing-wizard-footer"><button type="button" className="secondary" onClick={() => setStep((current) => current - 1)}>السابق</button><span /></footer> : null}
    </section>

    {selectedDay ? <div className="marketing-day-editor-overlay"><div className="marketing-day-editor marketing-agenda-day-editor"><header><div><h2>إضافة / تعديل الربط</h2><p>{new Date(`${selectedDay.date}T00:00:00Z`).toLocaleDateString("ar-SA", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</p></div><button type="button" onClick={() => setEditingDay(null)}>×</button></header><div className="marketing-agenda-editor-layout"><aside className="marketing-agenda-add-creative"><h3>إضافة كرييتيف جديد</h3><label><span>نوع الكرييتيف</span><select value={newCreativeTypeId} onChange={(event) => setNewCreativeTypeId(event.target.value)}><option value="">اختر النوع</option>{meta.creativeTypes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label><span>العدد</span><input type="number" min={1} value={newCreativeQuantity} onChange={(event) => setNewCreativeQuantity(Math.max(1, Number(event.target.value) || 1))} /></label><button type="button" className="primary" onClick={addCreativeToSelectedDay}><Plus size={17} />إضافة الكرييتيف لليوم</button><div className="marketing-agenda-add-note">القسم الأساسي يتحدد تلقائيًا حسب نوع الكرييتيف.</div></aside><main className="marketing-day-editor-body marketing-agenda-creatives-body"><div className="marketing-agenda-editor-toolbar"><strong>{selectedDay.creatives.length.toLocaleString("ar-SA")} كرييتيف مضاف</strong><button type="button" className="secondary" onClick={addCreativeToSelectedDay}><Plus size={16} />إضافة كرييتيف آخر</button></div>{selectedDay.creatives.map((creative, index) => <CreativeEditor key={creative.tempId} value={creative} meta={meta} showPlatforms hideBasics carsModal onChange={(value) => updateDay(selectedDay.date, selectedDay.creatives.map((item, itemIndex) => itemIndex === index ? value : item))} onDelete={() => updateDay(selectedDay.date, selectedDay.creatives.filter((_, itemIndex) => itemIndex !== index))} />)}{!selectedDay.creatives.length ? <div className="marketing-empty">اختر نوع الكرييتيف والعدد ثم اضغط إضافة الكرييتيف لليوم.</div> : null}</main></div><footer><button type="button" className="primary" onClick={() => setEditingDay(null)}>حفظ والعودة للجدول</button></footer></div></div> : null}
  </MarketingPage>;
}
