...
import { ArrowCounterClockwise, CaretDown, Copy, FloppyDisk, MagnifyingGlass, Plus, ShieldCheck, Trash, UserCircle, WarningCircle } from "@phosphor-icons/react";
...
type RoleGroup = { key: string; name: string; canonical: RoleItem; roleIds: string[]; permissionCodes: string[]; usersCount: number };
...
function groupRolesByDisplayName(roles: RoleItem[]): RoleGroup[] {
  ...
  return [...grouped.entries()].map(([key, items]) => {
    ...
    return {
      ...
      permissionCodes: [...new Set(sorted.flatMap((role) => role.permission_codes || []))],
      usersCount: sorted.reduce((sum, role) => sum + Number(role.users_count || 0), 0),
    };
...
export function UsersPermissionsPanel() {
  ...
  function isSectionOpen(key: string, fallback = false) {
    return openSections[key] ?? fallback;
  }
  function setSectionOpen(key: string, isOpen: boolean) {
    setOpenSections((current) => current[key] === isOpen ? current : { ...current, [key]: isOpen });
  }
  ...
  async function copyAccessFromUser() {
    ...
    try {
      ...
      setForm((current) => ({ ...current, roleIds: normalizeSelectedRoleIds(detail.roleIds || []), systems, overrides, reason: current.reason || `نسخ إعدادات الوصول من ${detail.user.full_name}` }));
      ...
      {tab === "users" ? <div className="access-users-grid">
        ...
        <section className="panel access-user-editor">
          ...
          <details className="access-editor-section access-account-section access-collapsible" open={isSectionOpen("account", true)} onToggle={(event) => setSectionOpen("account", event.currentTarget.open)}>
            <summary className="access-collapsible-summary"><div><h3>بيانات الحساب</h3><p>المعلومات الأساسية وحالة استقبال العملاء والتاسكات.</p></div><CaretDown size={18} /></summary>
            <div className="access-collapsible-content"><div className="access-basic-grid">
          ...
          <details className="access-fieldset access-collapsible" open={isSectionOpen("roles")} onToggle={(event) => setSectionOpen("roles", event.currentTarget.open)}>
            <summary className="access-collapsible-summary"><span>الأدوار العامة</span><CaretDown size={18} /></summary>
            <fieldset className="access-details-fieldset" disabled={!canManagePermissions}><div className="access-check-grid">{roleGroups.map((group) => { const checked = group.roleIds.some((id) => form.roleIds.includes(id)); return <label key={group.key}><input type="checkbox" checked={checked} onChange={() => setForm({ ...form, roleIds: checked ? form.roleIds.filter((id) => !group.roleIds.includes(id)) : [...form.roleIds.filter((id) => !group.roleIds.includes(id)), group.canonical.id] })} />{group.name}</label>; })}</div></fieldset>
          </details>
          <details className="access-central-permissions access-collapsible" open={isSectionOpen("core")} onToggle={(event) => setSectionOpen("core", event.currentTarget.open)}>
            <summary className="access-collapsible-summary"><span>صلاحيات المنصة والإعدادات المركزية</span><CaretDown size={18} /></summary>
            <div className="access-collapsible-content"><div className="access-effective-summary"><span>الموروثة: <strong>{previewPermissions.inheritedPermissions.filter((code) => corePermissions.some((permission) => permission.code === code)).length}</strong></span><span>السماح الفردي: <strong>{previewPermissions.directPermissions.filter((code) => corePermissions.some((permission) => permission.code === code)).length}</strong></span><span>المنع الفردي: <strong>{previewPermissions.deniedPermissions.filter((code) => corePermissions.some((permission) => permission.code === code)).length}</strong></span></div><div className="access-permission-pages">{corePages.map((page) => <details className="access-permission-page" key={page.code} open={isSectionOpen(`core-page:${page.code}`)} onToggle={(event) => setSectionOpen(`core-page:${page.code}`, event.currentTarget.open)}><summary><h3>{page.name_ar}</h3><CaretDown size={16} /></summary><div className="access-permission-page-content">{corePermissions.filter((permission) => permission.page_code === page.code).map((permission) => { const inherited=previewPermissions.inheritedPermissions.includes(permission.code); const direct=previewPermissions.directPermissions.includes(permission.code); const denied=previewPermissions.deniedPermissions.includes(permission.code); return <label key={permission.code} className={permission.is_sensitive ? "sensitive" : ""}><span><strong>{permission.name_ar}</strong><small>{permission.description_ar}</small><em className={denied ? "denied" : direct ? "direct" : inherited ? "inherited" : "none"}>{denied ? "ممنوعة فرديًا" : direct ? "مضافة فرديًا" : inherited ? "موروثة من الدور" : "غير ممنوحة"}</em></span><select disabled={!canManagePermissions} value={form.overrides[permission.code] || "inherit"} onChange={(event) => setForm({ ...form, overrides: { ...form.overrides, [permission.code]: event.target.value as OverrideEffect } })}><option value="inherit">{inherited ? "استخدام صلاحية الدور" : "بدون استثناء فردي"}</option><option value="allow">سماح فردي</option><option value="deny">منع فردي</option></select></label>; })}</div></details>)}</div></div>
          </details>
          ...
          <details className="access-system-config access-collapsible" open={isSectionOpen("system", true)} onToggle={(event) => setSectionOpen("system", event.currentTarget.open)}>
            <summary className="access-collapsible-summary"><span>إعدادات وصلاحيات {systemLabel(bootstrap, systemTab)}</span><CaretDown size={18} /></summary>
            <div className="access-collapsible-content">
              ...
              <details className="access-fieldset access-collapsible access-system-subsection" open={isSectionOpen(`system-departments:${systemTab}`)} onToggle={(event) => setSectionOpen(`system-departments:${systemTab}`, event.currentTarget.open)}>
                ...
                <fieldset className="access-details-fieldset" disabled={!canManagePermissions || systemTab === "marketing"}><div className="access-check-grid">{(bootstrap?.departments || []).filter((item) => item.is_active && (!item.system_code || item.system_code === systemTab)).map((department) => <label key={department.id}><input type="checkbox" checked={currentSystem.departmentIds.includes(department.id)} onChange={() => { const departmentIds=toggle(currentSystem.departmentIds, department.id); updateSystem({ departmentIds, primaryDepartmentId: departmentIds.includes(currentSystem.primaryDepartmentId) ? currentSystem.primaryDepartmentId : departmentIds[0] || "" }); }} />{department.name}</label>)}</div>{currentSystem.departmentIds.length ? <label className="access-primary-select"><span>القسم الأساسي</span><select value={currentSystem.primaryDepartmentId || currentSystem.departmentIds[0]} onChange={(event) => updateSystem({ primaryDepartmentId: event.target.value })}>{(bootstrap?.departments || []).filter((item) => currentSystem.departmentIds.includes(item.id)).map((department) => <option key={department.id} value={department.id}>{department.name}</option>)}</select></label> : null}</fieldset>
              </details>
              <div className="access-effective-summary"><span>الموروثة من الأدوار: <strong>{previewPermissions.inheritedPermissions.filter((code) => systemPermissions.some((permission) => permission.code === code)).length}</strong></span><span>السماح الفردي: <strong>{previewPermissions.directPermissions.filter((code) => systemPermissions.some((permission) => permission.code === code)).length}</strong></span><span>المنع الفردي: <strong>{previewPermissions.deniedPermissions.filter((code) => systemPermissions.some((permission) => permission.code === code)).length}</strong></span></div><div className="access-permission-pages">{pages.map((page) => <details className="access-permission-page" key={page.code} open={isSectionOpen(`system-page:${systemTab}:${page.code}`)} onToggle={(event) => setSectionOpen(`system-page:${systemTab}:${page.code}`, event.currentTarget.open)}><summary><h3>{page.name_ar}</h3><CaretDown size={16} /></summary><div className="access-permission-page-content">{systemPermissions.filter((permission) => permission.page_code === page.code).map((permission) => { const inherited=previewPermissions.inheritedPermissions.includes(permission.code); const direct=previewPermissions.directPermissions.includes(permission.code); const denied=previewPermissions.deniedPermissions.includes(permission.code); return <label key={permission.code} className={permission.is_sensitive ? "sensitive" : ""}><span><strong>{permission.name_ar}</strong><small>{permission.description_ar}</small><em className={denied ? "denied" : direct ? "direct" : inherited ? "inherited" : "none"}>{denied ? "ممنوعة فرديًا" : direct ? "مضافة فرديًا" : inherited ? "موروثة من الدور" : "غير ممنوحة"}</em></span><select disabled={!canManagePermissions} value={form.overrides[permission.code] || "inherit"} onChange={(event) => setForm({ ...form, overrides: { ...form.overrides, [permission.code]: event.target.value as OverrideEffect } })}><option value="inherit">{inherited ? "استخدام صلاحية الدور" : "بدون استثناء فردي"}</option><option value="allow">سماح فردي</option><option value="deny">منع فردي</option></select></label>; })}</div></details>)}</div>
            </div>
          </details>
