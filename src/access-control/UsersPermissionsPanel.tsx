    ...
    <div className="access-control-shell">
      ...
      {tab === "users" ? <div className="access-users-grid">
        ...
        <section className="panel access-user-editor">
          <div className="settings-card-title access-editor-title"><div><span className="access-title-icon"><ShieldCheck size={22} /></span><span><h2>{form.id ? "تعديل المستخدم" : "إضافة مستخدم"}</h2><p>{form.id ? `تحديث بيانات وصلاحيات ${selectedUser?.full_name || form.fullName}` : "إنشاء حساب جديد وتحديد الأدوار ونطاقات الوصول"}</p></span></div>{form.id ? <span className={`access-editor-status ${form.isActive ? "active" : "inactive"}`}>{form.isActive ? "حساب فعال" : "حساب موقوف"}</span> : <span className="access-editor-status new">حساب جديد</span>}</div>
