...
import { ArrowCounterClockwise, Copy, FloppyDisk, MagnifyingGlass, Plus, ShieldCheck, UserCircle, WarningCircle } from "@phosphor-icons/react";
...
export function UsersPermissionsPanel() {
  ...
  const canEditProfile = form.id ? canUpdateUsers : canCreateUsers;
  ...
  const canReadUsers = canViewUsers || canUpdateUsers || canDisableUsers || canManagePermissions;
  const canSaveUser = form.id ? (canUpdateUsers || canDisableUsers || canManagePermissions) : canCreateUsers;
  const canOpenPanel = canViewUsers || canCreateUsers || canUpdateUsers || canDisableUsers || canManageRoles || canManagePermissions || canManageOrg || canViewAudit || canViewSecurity;
  const availableTabs = (Object.keys(tabLabels) as Tab[]).filter((key) => {
    if (key === "users") return canViewUsers || canCreateUsers || canUpdateUsers || canDisableUsers || canManagePermissions;
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((item) => {
      ...
      if (filterRoleId && !cleanArray(item.role_ids).includes(filterRoleId) && !Object.values(item.systems || {}).some((system) => system.roleId === filterRoleId)) return false;
  ...
  }, [users, search, filterRoleId, filterSystemCode, filterBranchId, filterDepartmentId]);
  ...
  async function editUser(id: string) {
    ...
    try {
      ...
      setForm({ id: detail.user.id, employeeNo: detail.user.employee_no || "", fullName: detail.user.full_name, email: detail.user.email || "", mobile: detail.user.mobile || "", nextErpUserId: detail.user.next_erp_user_id || "", password: "", isActive: detail.user.is_active, canReceiveLeads: detail.user.can_receive_leads, canReceiveTasks: detail.user.can_receive_tasks, roleIds: detail.roleIds, systems, overrides, reason: "" });
    ...
    <div className="access-control-shell">
      ...
      {tab === "users" ? <div className="access-users-grid">
        <section className="panel access-users-list">
          <div className="settings-card-title"><div><UserCircle size={22} /><h2>المستخدمون</h2></div>{canCreateUsers ? <button type="button" className="secondary-button" onClick={() => { setForm(emptyForm(bootstrap)); setCopySourceId(""); }}><Plus size={17} /> مستخدم جديد</button> : null}</div>
          ...
          <div className="access-user-filter-grid"><select value={filterRoleId} onChange={(event) => setFilterRoleId(event.target.value)}><option value="">كل الأدوار</option>{(bootstrap?.roles || []).map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select><select value={filterSystemCode} onChange={(event) => setFilterSystemCode(event.target.value as "" | PlatformSystem)}><option value="">كل الأنظمة</option>{systemOrder.map((code) => <option key={code} value={code}>{systemLabel(bootstrap, code)}</option>)}</select><select value={filterBranchId} onChange={(event) => setFilterBranchId(event.target.value)}><option value="">كل الفروع</option>{(bootstrap?.branches || []).filter((item) => item.is_active).map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select><select value={filterDepartmentId} onChange={(event) => setFilterDepartmentId(event.target.value)}><option value="">كل الأقسام</option>{(bootstrap?.departments || []).filter((item) => item.is_active).map((department) => <option key={department.id} value={department.id}>{department.name}</option>)}</select></div>
          <div className="access-user-cards">{filteredUsers.map((item) => <button key={item.id} type="button" className={form.id === item.id ? "selected" : ""} onClick={() => void editUser(item.id)}><strong>{item.full_name}</strong><small>{item.email || item.mobile || "—"}</small><span>{item.roles || "بدون دور"}</span><em>{item.is_active ? "فعال" : "موقوف"}</em>{item.last_access_change_at ? <small className="access-last-change">آخر تعديل: {item.last_access_changed_by || "النظام"} · {new Date(item.last_access_change_at).toLocaleString("ar-SA")}</small> : null}</button>)}</div>
        </section>
        <section className="panel access-user-editor">
          <div className="settings-card-title"><div><ShieldCheck size={22} /><h2>{form.id ? "تعديل المستخدم" : "إضافة مستخدم"}</h2></div></div>
          ...
          <div className="access-basic-grid">
          ...
          <div className="access-toggle-row"><label><input type="checkbox" checked={form.isActive} disabled={Boolean(form.id) ? !canDisableUsers : !canEditProfile} onChange={(event) => setForm({ ...form, isActive: event.target.checked })} /> الحساب فعال</label><label><input type="checkbox" disabled={!canEditProfile} checked={form.canReceiveLeads} onChange={(event) => setForm({ ...form, canReceiveLeads: event.target.checked })} /> استقبال العملاء</label><label><input type="checkbox" disabled={!canEditProfile} checked={form.canReceiveTasks} onChange={(event) => setForm({ ...form, canReceiveTasks: event.target.checked })} /> استقبال التاسكات</label></div>
          <fieldset className="access-fieldset" disabled={!canManagePermissions}><legend>الأدوار العامة</legend><div className="access-check-grid">{(bootstrap?.roles || []).map((role) => <label key={role.id}><input type="checkbox" checked={form.roleIds.includes(role.id)} onChange={() => setForm({ ...form, roleIds: toggle(form.roleIds, role.id) })} />{role.name}</label>)}</div></fieldset>
          ...
          <label><span>سبب التعديل</span><textarea value={form.reason} onChange={(event) => setForm({ ...form, reason: event.target.value })} placeholder="يسجل داخل سجل تعديلات الصلاحيات" /></label>
          <button type="button" className="primary-button access-save" disabled={saving || !canSaveUser || !form.fullName} onClick={() => void saveUser()}><FloppyDisk size={18} />{saving ? "جاري الحفظ..." : "حفظ المستخدم والصلاحيات"}</button>
        </section>
      </div> : null}
