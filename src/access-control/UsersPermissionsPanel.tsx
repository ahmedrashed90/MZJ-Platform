...
export function UsersPermissionsPanel() {
  ...
  return (
    <div className="access-control-shell">
      ...
      {tab === "roles" ? <div className="access-role-grid"><section className="panel"><div className="settings-card-title"><h2>قوالب الأدوار</h2><button type="button" className="secondary-button" onClick={() => chooseRole(null)}><Plus size={17} /> دور جديد</button></div>{roleGroups.map((group) => { const role=group.canonical; return <button type="button" className={`access-role-row ${editingRole?.id === role.id ? "selected" : ""}`} key={group.key} onClick={() => chooseRole(role)}><strong>{group.name}</strong><small>{role.code} · {role.permission_codes.length} صلاحية · {group.roleIds.reduce((total,id)=>total+(bootstrap?.roles.find((item)=>item.id===id)?.users_count||0),0)} مستخدم</small></button>; })}</section><section className="panel"><h2>{editingRole ? "تعديل قالب الدور" : "إنشاء قالب دور"}</h2><div className="access-basic-grid"><label><span>الكود</span><input value={roleDraft.code} onChange={(event) => setRoleDraft({ ...roleDraft, code: event.target.value })} /></label><label><span>الاسم</span><input value={roleDraft.name} onChange={(event) => setRoleDraft({ ...roleDraft, name: event.target.value })} /></label></div><label><span>الوصف</span><textarea value={roleDraft.description} onChange={(event) => setRoleDraft({ ...roleDraft, description: event.target.value })} /></label><div className="access-role-permissions">{roleSystemOrder.map((code) => <section key={code}><h3>{systemLabel(bootstrap, code)}</h3>{(bootstrap?.permissions || []).filter((item) => item.system_code === code).map((permission) => <label key={permission.code}><input type="checkbox" disabled={!canManagePermissions} checked={roleDraft.permissionCodes.includes(permission.code)} onChange={() => setRoleDraft({ ...roleDraft, permissionCodes: toggle(roleDraft.permissionCodes, permission.code) })} />{permission.name_ar}</label>)}</section>)}</div><button type="button" className="primary-button access-save" disabled={saving || !roleDraft.code || !roleDraft.name} onClick={() => void saveRole()}><FloppyDisk size={18} /> حفظ قالب الدور</button></section></div> : null}
