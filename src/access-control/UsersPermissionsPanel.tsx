...
export function UsersPermissionsPanel() {
  ...
  return (
    <div className="access-control-shell">
        ...
        <section className="panel access-user-editor">
          ...
          <details className="access-system-config access-collapsible" open={isSectionOpen("system", true)} onToggle={(event) => setSectionOpen("system", event.currentTarget.open)}>
            ...
            <div className="access-collapsible-content">
              ...
              {systemTab === "operations" ? <details className="access-fieldset access-collapsible access-system-subsection" open={isSectionOpen("system-vehicle-statuses:operations")} onToggle={(event) => setSectionOpen("system-vehicle-statuses:operations", event.currentTarget.open)}>
                <summary className="access-collapsible-summary"><span>حالات السيارات المسموحة</span><CaretDown size={17} /></summary>
                <fieldset className="access-details-fieldset" disabled={!canManagePermissions}>
                  <small className="access-scope-note">عدم تحديد حالات يعني السماح بكل حالات السيارات الفعالة.</small>
                  <div className="access-check-grid">{(bootstrap?.vehicleStatuses || []).filter((item) => item.is_active).map((vehicleStatus) => <label key={vehicleStatus.code}><input type="checkbox" checked={currentSystem.vehicleStatusCodes.includes(vehicleStatus.code)} onChange={() => updateSystem({ vehicleStatusCodes: toggle(currentSystem.vehicleStatusCodes, vehicleStatus.code) })} />{vehicleStatus.name}</label>)}</div>
                </fieldset>
              </details> : null}
      ...
      {tab === "roles" ? <div className="access-role-grid"><section className="panel"><div className="settings-card-title"><h2>قوالب الأدوار</h2><button type="button" className="secondary-button" onClick={() => chooseRole(null)}><Plus size={17} /> دور جديد</button></div>{roleGroups.map((group) => { const role=group.canonical; const usersCount=(bootstrap?.roles || []).filter((item) => group.roleIds.includes(item.id)).reduce((total, item) => total + Number(item.users_count || 0), 0); return <button type="button" className={`access-role-row ${editingRole?.id === role.id ? "selected" : ""}`} key={role.id} onClick={() => chooseRole(role)}><strong>{group.name}</strong><small>{role.code} · {role.permission_codes.length} صلاحية · {usersCount} مستخدم</small></button>; })}</section><section className="panel"><h2>{editingRole ? "تعديل قالب الدور" : "إنشاء قالب دور"}</h2><div className="access-basic-grid"><label><span>الكود</span><input value={roleDraft.code} onChange={(event) => setRoleDraft({ ...roleDraft, code: event.target.value })} /></label><label><span>الاسم</span><input value={roleDraft.name} onChange={(event) => setRoleDraft({ ...roleDraft, name: event.target.value })} /></label></div><label><span>الوصف</span><textarea value={roleDraft.description} onChange={(event) => setRoleDraft({ ...roleDraft, description: event.target.value })} /></label><div className="access-role-permissions">{roleSystemOrder.map((code) => <section key={code}><h3>{systemLabel(bootstrap, code)}</h3>{(bootstrap?.permissions || []).filter((item) => item.system_code === code).map((permission) => <label key={permission.code}><input type="checkbox" disabled={!canManagePermissions} checked={roleDraft.permissionCodes.includes(permission.code)} onChange={() => setRoleDraft({ ...roleDraft, permissionCodes: toggle(roleDraft.permissionCodes, permission.code) })} />{permission.name_ar}</label>)}</section>)}</div><button type="button" className="primary-button access-save" disabled={saving || !roleDraft.code || !roleDraft.name} onClick={() => void saveRole()}><FloppyDisk size={18} /> حفظ قالب الدور</button></section></div> : null}
