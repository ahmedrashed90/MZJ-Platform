          ...
          <details className="access-system-config access-collapsible" open={isSectionOpen("system", true)} onToggle={(event) => setSectionOpen("system", event.currentTarget.open)}>
            ...
            <div className="access-collapsible-content">
              ...
              <details className="access-fieldset access-collapsible access-system-subsection" open={isSectionOpen(`system-departments:${systemTab}`)} onToggle={(event) => setSectionOpen(`system-departments:${systemTab}`, event.currentTarget.open)}>
                ...
                <fieldset className="access-details-fieldset" disabled={!canManagePermissions}><div className="access-check-grid">{(bootstrap?.departments || []).filter((item) => item.is_active && (systemTab === "marketing" ? item.system_code === "marketing" : (!item.system_code || item.system_code === systemTab))).map((department) => <label key={department.id}><input type="checkbox" checked={currentSystem.departmentIds.includes(department.id)} onChange={() => { const departmentIds=toggle(currentSystem.departmentIds, department.id); updateSystem({ departmentIds, primaryDepartmentId: departmentIds.includes(currentSystem.primaryDepartmentId) ? currentSystem.primaryDepartmentId : departmentIds[0] || "" }); }} />{department.name}</label>)}</div>{currentSystem.departmentIds.length ? <label className="access-primary-select"><span>القسم الأساسي</span><select value={currentSystem.primaryDepartmentId || currentSystem.departmentIds[0]} onChange={(event) => updateSystem({ primaryDepartmentId: event.target.value })}>{(bootstrap?.departments || []).filter((item) => currentSystem.departmentIds.includes(item.id)).map((department) => <option key={department.id} value={department.id}>{department.name}</option>)}</select></label> : null}</fieldset>
              </details>
