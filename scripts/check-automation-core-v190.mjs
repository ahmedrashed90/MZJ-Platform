...
for (const token of ["crm.contacts", "crm.contact_identities", "crm.service_requests", "crm.automation_rules", "crm.automation_jobs", "crm.ownership_events", "crm.media_assets", "crm-automation-core-v1.9"]) {
  if (!schema.includes(token)) throw new Error(`Automation schema v1.9 check failed: missing ${token}`);
}
for (const token of ["sendServiceSelection", "classifyConversationService", "scheduleInboxAgent", "processDueAutomationJobs", "human_reply_detected", "branch_manager_escalation", "sales_manager_escalation"]) {
...
for (const token of ["crm/automations", "crm/inbox", "crm/ownership", "crm/media", "integrations/media", "cron/automation"]) {
...
console.log("CRM automation core v1.9 and Mersal transport-only Worker checks passed.");
