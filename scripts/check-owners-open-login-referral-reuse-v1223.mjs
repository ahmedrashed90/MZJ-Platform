...
const checks = [
  ['schema state includes registered-referral support', schema.includes('version=greatest(version,1226)') && schema.includes('>= 1226')],
  ...
  ['OTP request accepts member or new-customer code record', publicApi.includes('const legacyCustomer = member ? null : await findLegacyCustomerCodeByPhone(phone);') && publicApi.includes('رقم الجوال غير مسجل في MZJ Club Community')],
  ...
  ['public login copy no longer requires completed purchase', portal.includes('المسجل في MZJ Club Community') && !portal.includes('المسجل في عملية الشراء')],
  ['new-customer portal exposes invite controls only when API returns a working URL', portal.includes('{member.inviteUrl ? (') && publicApi.includes('inviteUrl: legacyCustomer.referral_code ?')],
  ['registered customer referrals have a dedicated pre-sale owner column', schema.includes('referrer_legacy_customer_code_id uuid references owners.legacy_customer_codes')],
  ['public invite lookup accepts active registered-customer codes', publicApi.includes('findPublicInviteReferrer') && publicApi.includes('findLegacyCustomerCodeByCode(code)')],
  ['registered-customer invite registration stays attached to the legacy identity', publicApi.includes('referrer_legacy_customer_code_id,referred_name') && publicApi.includes('referrer.referrer_kind === "legacy" ? referrer.id : null')],
  ['pre-sale referral ownership migrates to the sold member on first purchase', core.includes('legacyConversion.transferredReferrals') && core.includes('reconcileOwnerReferralPointsForMember')],
];
