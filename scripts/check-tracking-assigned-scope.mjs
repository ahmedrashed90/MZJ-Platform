...
const counts = read("server/_tracking-counts.ts");
...
expect("Order lists and details require the current assignee for assigned scope", (orders.match(/scope\.assignedOnly}=true and o\.assigned_to=\$\{user\.id\}::uuid/g) || []).length >= 3 && orders.includes("getTrackingCountSummary(sql, user, { from, to })"));
...
expect("Canonical Tracking counters use the same assignee", counts.includes("scope.assignedOnly}=true and o.assigned_to=${user.id}::uuid"));
expect("Unified dashboard tracking metrics use the canonical scoped counters", dashboard.includes("getTrackingCountSummary(sql, user)"));
