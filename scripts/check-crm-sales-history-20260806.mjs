...
assert(reports.includes("const salesFacts = await sql") && reports.includes("from crm.sales_transactions st"), "reports do not use canonical sale transactions");
assert(!reports.includes("const erpSalesFacts = await sql") && !reports.includes("const salesFacts = [...erpSalesFacts"), "reports still double-count ERP orders beside canonical transactions");
