...
const checks = [
  ...
  ["Direct upload sessions are database-backed", files.schema.includes("marketing.zoho_upload_tickets") && files.marketing.includes("confirmFinalUpload") && files.marketing.includes("cancelFinalUpload")],
  ...
  ["Browser uploads directly to Zoho", files.client.includes("new XMLHttpRequest()") && files.client.includes("/workdrive/api/v1/upload") && files.client.includes("/workdrive-api/v1/stream/upload")],
  ...
  ["Carousel server records remain ordered", files.marketing.includes("multipleImages") && files.marketing.includes("media_type:'CAROUSEL'")],
  ...
  ["Zoho connection UI", files.connections.includes("Zoho WorkDrive") && files.connections.includes("/api/integrations/zoho/start") && files.connections.includes("رفع مباشر")],
