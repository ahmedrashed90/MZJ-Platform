...
const checks = [
  ...
  ["Platform proxy staging is database-backed and bounded", files.schema.includes("marketing.zoho_standard_upload_parts") && files.schema.includes("byte_length <= 4194304") && files.marketing.includes("stageFinalUploadPart") && files.marketing.includes("readFinalUploadParts")],
  ...
  ["Upload strategy keeps standard upload below 64 MiB and chunk sessions from 64 MiB", files.upload.includes("ZOHO_CHUNK_UPLOAD_MIN_FILE_SIZE = 64 * 1024 * 1024") && files.upload.includes("input.fileSize < ZOHO_CHUNK_UPLOAD_MIN_FILE_SIZE") && files.marketing.includes("upload_strategy")],
  ...
  ["Zoho provider chunks are aggregated into 64 MiB windows", files.upload.includes("ZOHO_PROVIDER_CHUNK_SIZE = 64 * 1024 * 1024") && files.marketing.includes("readFinalUploadRange") && files.marketing.includes("flushFinalUploadZohoChunks") && files.upload.includes("جزء Zoho غير النهائي يجب أن يكون 64 MiB كاملًا")],
