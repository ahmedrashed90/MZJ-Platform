...
check("existing CRM source is repaired only for customers linked to stored social engagement", backend.includes("repairStoredEngagementSources") && backend.includes("20260730_repair_engagement_crm_sources") && backend.includes("pe.processing_status in ('created','reused')") && backend.includes("pe.engagement_type in ('comment','like')") && backend.includes("then 'بوست انستجرام' else 'بوست فيس بوك'"));
check("engagement API excludes deleted rows", backend.includes("where pp.is_deleted=false") && backend.includes("where pe.is_deleted=false and pe.engagement_type in ('comment','like') and pp.is_deleted=false"));
