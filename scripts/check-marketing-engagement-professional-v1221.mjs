...
check("engagement API excludes deleted rows", backend.includes("where pp.is_deleted=false") && backend.includes("where pe.is_deleted=false and pp.is_deleted=false"));
...
check("page has professional shared filters", page.includes("marketing-engagement-control-panel") && page.includes("postStatus") && page.includes("engagementKind"));
...
check("engagements expose archive restore delete and customer delete", page.includes('manage("engagement", "archive"') && page.includes('manage("engagement", "delete_customer"'));
