...
check("attendance report has no automatic interval refresh", !report.includes("setInterval") && !report.includes("clearInterval"));
...
check("day blocks use alternating visual tones", styles.includes("day-tone-0") || (styles.includes("day-tone-1") && report.includes("groupIndex % 2")));
check("report export supports grouped blocks", report.includes('id="attendance-report-export"') && report.includes("content.outerHTML"));
...
console.log(`\nGlobal attendance + Device Agent v90 checks: ${checks.length - failed}/${checks.length} passed.`);
