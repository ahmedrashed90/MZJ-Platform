...
check("excel export refetches current filters", report.includes("fetchReportPayload(from, to, employeeIds)") && report.includes("buildExcelDocument(payload)") && !report.includes("content.outerHTML"));
...
console.log(`\nGlobal attendance + Device Agent + report v91 checks: ${checks.length - failed}/${checks.length} passed.`);
