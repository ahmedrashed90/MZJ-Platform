...
check("browser GPS targets fifteen meters and supports desktop fallback to seventy-five meters", locationHelper.includes("TARGET_ATTENDANCE_ACCURACY_M = 15") && locationHelper.includes("MAX_DESKTOP_ATTENDANCE_ACCURACY_M = 75"));
...
check("browser GPS keeps sampling desktop readings and rejects only worse than seventy-five meters", locationHelper.includes("bestLocation.accuracy <= MAX_DESKTOP_ATTENDANCE_ACCURACY_M") && locationHelper.includes("الحد المقبول ±75م"));
check("server independently rejects desktop readings worse than seventy-five meters", service.includes("MAX_ATTENDANCE_ACCURACY_M = 75") && service.includes("ATTENDANCE_LOCATION_ACCURACY_LOW") && service.includes("accuracy > MAX_ATTENDANCE_ACCURACY_M"));
...
check("report shows captured accuracy and branch distance", report.includes("دقة ±") && report.includes("بعد "));
...
console.log(`\nGlobal attendance v85 checks: ${checks.length - failed}/${checks.length} passed.`);
