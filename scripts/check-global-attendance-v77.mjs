...
check("attendance safe transition setting exists and defaults off", schema.includes("core.attendance_settings") && schema.includes("enforcement_enabled boolean not null default false"));
check("login bypasses attendance while enforcement is disabled", service.includes("if (!(await isAttendanceEnforcementEnabled()))") && service.includes("return { enforced: false, checkedIn: false, state: null }"));
check("existing sessions stay allowed while enforcement is disabled", service.includes("if (!(await isAttendanceEnforcementEnabled())) return true"));
check("attendance cron is paused while enforcement is disabled", service.includes("enforcementEnabled: false") && service.includes("forcedLogoutUsers: 0"));
check("admin can explicitly enable attendance enforcement", attendanceSettings.includes("تطبيق الحضور على تسجيل الدخول") && attendanceSettings.includes("save_settings") && api.includes('action === "save_settings"'));
...
console.log(`\nGlobal attendance v77 checks: ${checks.length - failed}/${checks.length} passed.`);
