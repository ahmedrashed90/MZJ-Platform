...
const checks = [
  ...
  ["publish prep is driven by execution tasks, including completed tasks without schedule rows", marketingApi.includes("from marketing.tasks t") && marketingApi.includes("t.task_kind='execution'") && marketingApi.includes("coalesce(schedule_row.group_id::text,t.id::text) as id") && marketingApi.includes("cam.status in ('ready_publish','publishing')") && marketingApi.includes("ag.status in ('ready_publish','publishing')") && !marketingApi.includes("t.status<>'completed'")],
  ["publish prep save accepts ready-to-publish and publishing execution tasks", (marketingApi.match(/cam\.status in \('ready_publish','publishing'\)/g) || []).length >= 2 && (marketingApi.match(/ag\.status in \('ready_publish','publishing'\)/g) || []).length >= 2],
