...
expect("purchase award SQL parameters are explicitly typed before JSON construction", (core.match(/purchaseAwardPoints',\$\{configuredPoints\}::integer/g) || []).length >= 2 && core.includes("inserted_counts.awards*${configuredPoints}::integer"));
...
expect("commerce API maps buyer-owned and friend-owned codes to separate reward audiences", publicApi.includes("rewardAvailableForReferrerKind") && publicApi.includes("referrerKind === \"legacy\"") && publicApi.includes("available_for_referral_purchase === true") && publicApi.includes("available_for_existing_customer_purchase === true") && publicApi.includes("available_for_friend_referral_purchase === true") && publicApi.includes("eligibility.selfUse === true") && publicApi.includes("مكافأة العميل الجديد المختارة لم تعد متاحة") && publicApi.includes("مكافأة العميل القديم المختارة لم تعد متاحة") && publicApi.includes("مكافأة دعوة من صديق المختارة لم تعد متاحة") && publicApi.includes('action === "commerce_confirm_bundle"'));
