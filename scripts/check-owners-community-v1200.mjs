...
expect("Owners schema readiness is independent from deprecated Mersal OTP columns", !schema.includes("column_name='otp_channel'") && schema.includes(">= 1221"));
...
expect("admin membership preview reuses the exact customer discount calculator", memberPreview.includes("OwnersDiscountCalculator") && portal.includes("OwnersDiscountCalculator") && discountCalculator.includes("احسب خصمك") && discountCalculator.includes("priceBeforeTax") && discountCalculator.includes("* 0.01") && discountCalculator.includes("Math.ceil(rawDiscount / 100) * 100") && adminApi.includes("websiteCarsForDiscountCalculator") && adminApi.includes("getWebsiteStock") && adminApi.includes("websiteCars: websiteCars.cars"));
...
expect("customer point and referral labels use the requested copy", portal.includes("إرسال دعوة لصديق - تم الشراء") && portal.includes("إعادة الشراء") && portal.includes("owners-referral-ledger") && memberPreview.includes("إرسال دعوة لصديق - تم الشراء") && memberPreview.includes("owners-referral-ledger"));
...
expect("Owners schema readiness gates the personal-code ledger and website redemption columns before public commerce runs", schema.includes("table_name='personal_code_uses'") && ["website_order_id", "next_erp_sales_order", "used_channel", "used_by_phone_normalized"].every((column) => schema.includes(`table_name='redemptions' and column_name='${column}'`)) && schema.includes("values(1,1221,now())") && schema.includes(">= 1221"));
...
expect("member portal exposes points, invite link, referrals, rewards and ledger", ["member.inviteUrl", "owners-public-rewards", "owners-referral-ledger", "owners-ledger"].every((text) => portal.includes(text)));
