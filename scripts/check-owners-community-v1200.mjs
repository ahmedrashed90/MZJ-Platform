...
expect("Owners schema readiness is independent from deprecated Mersal OTP columns", !schema.includes("column_name='otp_channel'") && schema.includes(">= 1207"));
...
expect("reward catalog can expose selected rewards to referral-code website purchases", schema.includes("available_for_referral_purchase") && adminApi.includes("availableForReferralPurchase") && adminPage.includes("متاحة للعميل الجديد عند استخدام كود الدعوة في طلب الشراء"));
expect("website discount rewards store a structured SAR amount separate from display text", schema.includes("checkout_discount_amount") && adminApi.includes("checkoutDiscountAmount") && publicApi.includes("discountAmount"));
expect("referral purchase benefits are audited once per customer and website order", schema.includes("owners.referral_purchase_benefits") && schema.includes("unique(referred_phone_normalized)") && schema.includes("unique(website_order_id)"));
expect("commerce API validates referral eligibility without replacing the existing referral flow", publicApi.includes('action === "commerce_rewards"') && publicApi.includes("commerceEligibility") && publicApi.includes("registerReferralCore") && publicApi.includes('source: "website_purchase"'));
