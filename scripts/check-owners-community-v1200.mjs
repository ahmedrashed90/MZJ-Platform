...
expect("Owners Community uses PostgreSQL and adds no Firebase dependency", ![schema, core, publicApi, adminApi].some((source) => /firebase|firestore/i.test(source)));
...
expect("OTP uses an approved active Mersal template", publicApi.includes("deliverDirectWhatsapp") && publicApi.includes("upper(coalesce(status,''))='APPROVED'") && publicApi.includes("otp_template_id"));
