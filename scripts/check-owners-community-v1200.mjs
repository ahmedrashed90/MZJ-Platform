...
expect("Owners schema readiness is independent from deprecated Mersal OTP columns", !schema.includes("column_name='otp_channel'") && schema.includes(">= 1223"));
...
expect("Owners schema readiness gates the personal-code ledger and website redemption columns before public commerce runs", schema.includes("table_name='personal_code_uses'") && ["website_order_id", "next_erp_sales_order", "used_channel", "used_by_phone_normalized"].every((column) => schema.includes(`table_name='redemptions' and column_name='${column}'`)) && schema.includes("values(1,0,now())") && schema.includes("current_version < 1222") && schema.includes(">= 1223"));
