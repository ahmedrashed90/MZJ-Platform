...
expect("new website buyers receive a designated new-customer reward plus at most one optional old-customer reward", publicApi.includes("primaryNewRewardId") && publicApi.includes('action === "commerce_confirm_bundle"') && publicApi.includes('rewardSlot: slot') && publicApi.includes('`${websiteOrderId}:bonus`'));
