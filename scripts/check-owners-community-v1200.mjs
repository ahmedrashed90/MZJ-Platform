...
const memberPreview = read("src/owners/OwnersMemberPreviewPage.tsx");
const discountCalculator = read("src/owners/OwnersDiscountCalculator.tsx");
...
const schemaReadyVersion = Number(schema.match(/return\s+Number\(state\?\.version\s*\|\|\s*0\)\s*>=\s*(\d+);/)?.[1] || 0);
...
expect("Owners schema readiness is independent from deprecated Mersal OTP columns", !schema.includes("column_name='otp_channel'") && schemaReadyVersion >= 1224);
...
expect("purchase award runtime parameters are typed once before JSON construction", core.includes("with runtime_values as (") && core.includes("${configuredPoints}::integer as purchase_award_points") && core.includes("${configuredRepurchasePoints}::integer as repurchase_award_points") && core.includes("'purchaseAwardPoints',case when sale.sale_rank=1") && !core.includes("'purchaseAwardPoints',${configuredPoints}"));
...
expect("points menu values are configurable for repurchase, referral sale and referral send", schema.includes("points_repurchase") && settingsPanel.includes("pointsRepurchase") && settingsPanel.includes("إعادة الشراء") && settingsPanel.includes("إرسال دعوة لصديق - تم الشراء") && settingsPanel.includes("إرسال دعوة لصديق"));
expect("v1222 resets the experimental ledger once to one 500-point opening row per member", schema.includes("current_version < 1222") && schema.includes("delete from owners.points_ledger") && schema.includes("pointsResetVersion',1222") && schema.includes("points_balance=500"));
expect("v1222 reset uses a member-owned unique event key and preserves historical sale metadata without sale-key collisions", schema.includes("'purchase:member:'||member.id::text||':initial'") && schema.includes("'pointsResetBaseline',true") && !schema.includes("then 'purchase:'||first_sales.sale_id::text") && core.includes("pointsResetBaseline"));
expect("public and admin member pages use the dynamic points menu and movement ledger", portal.includes("pointsMenu.repurchase") && memberPreview.includes("pointsMenu.repurchase") && portal.includes("سجل الحركة") && memberPreview.includes("سجل الحركة") && portal.includes("<span>النقاط</span>") && memberPreview.includes("<span>النقاط</span>"));
expect("public compact welcome and redemption badge block is removed", !portal.includes("owners-public-welcome-compact") && !portal.includes("طلب استبدال"));
...
expect("reward catalog explicitly controls point-based customer-page visibility", schema.includes("show_on_member_page boolean not null default false") && adminApi.includes("showOnMemberPage") && adminPage.includes("إظهار المكافأة في صفحة العميل حسب رصيد النقاط") && publicApi.includes("show_on_member_page=true"));
expect("old catalog rewards stay hidden from customer page unless explicitly enabled", schema.includes("show_on_member_page boolean not null default false") && publicApi.includes("where is_active=true and show_on_member_page=true"));
expect("sold and new customer lists open a permission-guarded membership preview", app.includes('path="/owners-community/member/:kind/:id"') && adminPage.includes('/owners-community/member/member/${member.id}') && adminPage.includes('/owners-community/member/legacy/${customer.id}') && memberPreview.includes('ownersAdminGet("profile"'));
expect("admin membership preview reuses the exact customer discount calculator", memberPreview.includes("OwnersDiscountCalculator") && portal.includes("OwnersDiscountCalculator") && discountCalculator.includes("اعرف خصمك") && discountCalculator.includes("priceBeforeTax") && discountCalculator.includes("discountAmount") && discountCalculator.includes("roundingUnit") && discountCalculator.includes("خصم إهداء لصديق") && discountCalculator.includes("ابحث عن السيارة") && adminApi.includes("websiteCarsForDiscountCalculator") && adminApi.includes("getWebsiteStock") && adminApi.includes("websiteCars: websiteCars.cars"));
expect("deprecated Owners Community delivery tab is removed from the Owners system", !adminPage.includes('tab === "delivery"') && !adminPage.includes("OwnersRedemptionDesk") && !adminPage.includes('setTab("delivery")') && !fs.existsSync("src/owners/OwnersRedemptionDesk.tsx"));
expect("customer point and referral labels use the requested copy", portal.includes("إرسال دعوة لصديق - تم الشراء") && portal.includes("إعادة الشراء") && portal.includes("owners-movement-table") && portal.includes("سجل الحركة") && memberPreview.includes("إرسال دعوة لصديق - تم الشراء") && memberPreview.includes("owners-movement-table") && memberPreview.includes("سجل الحركة"));
...
expect("existing real owners may reuse their own customer code on later old-customer purchases", publicApi.includes("selfUse") && publicApi.includes("No cross-order one-use check here") && !publicApi.includes("سبق استخدام كود الدعوة الخاص بك للاستفادة من مكافأة عميل قديم"));
expect("personal old-customer code keeps a per-order ledger and server-side 1 percent calculation while allowing repeat member purchases", schema.includes("owners.personal_code_uses") && schema.includes("drop constraint if exists personal_code_uses_member_id_key") && !schema.includes("unique(member_id),") && schema.includes("unique(website_order_id)") && publicApi.includes('action === "commerce_personal_code_use"') && publicApi.includes("personalCodeDiscountAmount") && publicApi.includes("where website_order_id=${websiteOrderId}"));
expect("Owners schema readiness gates the reusable personal-code ledger and website redemption columns before public commerce runs", schema.includes("table_name='personal_code_uses'") && ["website_order_id", "next_erp_sales_order", "used_channel", "used_by_phone_normalized"].every((column) => schema.includes(`table_name='redemptions' and column_name='${column}'`)) && schema.includes("values(1,0,now())") && schema.includes("current_version < 1222") && schemaReadyVersion >= 1224);
...
expect("checkout discovers only issued active reward-redemption slots by buyer phone without exposing codes", publicApi.includes('action === "commerce_redemptions_for_phone"') && publicApi.includes("rd.status='approved'") && publicApi.includes("rd.redemption_code is not null") && publicApi.includes("json({ ok: true, count: rows.length })"));
expect("checkout can atomically confirm multiple reward-redemption codes on one website order", publicApi.includes('action === "commerce_redemptions_confirm"') && publicApi.includes("for (const code of codes)") && schema.includes("drop index if exists owners.owners_redemptions_website_order_unique") && schema.includes("create index if not exists owners_redemptions_website_order_idx"));
...
expect("member portal exposes points, invite link, referrals, rewards and ledger", ["member.inviteUrl", "owners-public-rewards", "owners-movement-table", "owners-ledger"].every((text) => portal.includes(text)));
...
expect("member portal keeps the referral link inside the customer page without the points-explanation note", portal.includes("إرسال الدعوة لصديق") && portal.includes("owners-member-invite-card") && !portal.includes("كل صديق جديد يفتح الرابط أو يسجل أو يتأهل أو يشتري يضيف نقاطًا حسب إعدادات البرنامج."));
...
expect("sold and new customer tabs search locally by customer name or phone", adminPage.includes("membersSearch") && adminPage.includes("legacySearch") && adminPage.includes("ابحث باسم العميل أو رقم الجوال") && adminPage.includes("normalizeOwnerSearch(member.customer_name)") && adminPage.includes("normalizeOwnerSearch(customer.phone_normalized)"));
...
expect("membership back shows the customer category independently from catalog rewards", portal.includes("فئة العميل") && portal.includes("owners-card-back-details") && portal.includes("كود العميل") && portal.includes("عدد مرات الشراء") && !portal.includes("owners-card-back-rewards"));
