...
expect("commerce API maps new-customer-tab codes to new-customer rewards and sold-member codes to existing-customer rewards", publicApi.includes("rewardAvailableForReferrerKind") && publicApi.includes("referrerKind === \"legacy\"") && publicApi.includes("available_for_referral_purchase === true") && publicApi.includes("available_for_existing_customer_purchase === true") && publicApi.includes("getCommerceRewards(eligibility.referrerKind)") && publicApi.includes("مكافأة العميل الجديد المختارة لم تعد متاحة") && publicApi.includes("مكافأة العميل القديم المختارة لم تعد متاحة") && publicApi.includes('action === "commerce_confirm_bundle"'));
