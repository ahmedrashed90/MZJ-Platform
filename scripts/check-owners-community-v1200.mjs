...
expect("Owners schema readiness is independent from deprecated Mersal OTP columns", !schema.includes("column_name='otp_channel'") && schema.includes(">= 1209"));
...
expect("reward catalog can expose separate new-customer and existing-customer rewards to referral-code website purchases", schema.includes("available_for_referral_purchase") && schema.includes("available_for_existing_customer_purchase") && adminApi.includes("availableForExistingCustomerPurchase") && adminPage.includes("متاحة للعميل الجديد عند استخدام كود الدعوة في طلب الشراء") && adminPage.includes("متاحة للعميل القديم عند استخدام كود الدعوة في طلب الشراء"));
...
expect("commerce API classifies website buyers as new or existing while keeping the canonical referral flow for new customers", publicApi.includes('action === "commerce_rewards"') && publicApi.includes("customerKind") && publicApi.includes("registerReferralCore") && publicApi.includes('source: "website_purchase"'));
...
expect("commerce reward confirmation uses the canonical referral registration flow", publicApi.includes('action === "commerce_confirm"') && publicApi.includes("registerReferralCore(") && publicApi.includes("ownerReferralWebsiteOrderId"));
...
expect("commerce API exposes every active new-customer reward and bundle confirmation accepts the selected new-customer reward", publicApi.includes("newCustomerRewards") && publicApi.includes("newCustomerRewardIds") && publicApi.includes("available_for_referral_purchase === true") && publicApi.includes("مكافأة العميل الجديد المختارة لم تعد متاحة") && publicApi.includes('action === "commerce_confirm_bundle"'));
