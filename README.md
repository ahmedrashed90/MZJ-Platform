...
- توحيد Schema التسويق في ملف أساسي واحد: `database/marketing_native_schema.sql`.
- مزامنة نفس الـSchema حرفيًا مع Runtime Schema وملف الـMigration، مع فحص يمنع اختلاف النسخ الثلاث.
- ترقية كل جداول التسويق الموجودة بإضافة الأعمدة الناقصة قبل إنشاء الفهارس أو تشغيل استعلامات النظام.
- إصلاح جداول `marketing.activity_log` القديمة بإضافة `entity_type` وبقية الأعمدة المطلوبة مع الحفاظ على السجلات السابقة.
- تشغيل الترقية داخل Transaction واحدة مع PostgreSQL advisory lock، لمنع ترك قاعدة البيانات في حالة تحديث جزئي عند حدوث خطأ.
- التحقق من 28 جدولًا و295 عمودًا داخل PostgreSQL قبل اعتماد الترقية.
- إضافة اختبارات لمسارات الترقية من قاعدة فارغة، ومن Schema المنصة v1.18، ومن الحالات الجزئية التي ظهرت أثناء النشر.
...
- إصدار Node في المشروع موحّد على `22.x` طبقًا لـ `package.json` وإعداد البناء المعتمد.
...
## v1.19.0 - Native Marketing Rebuild

- إعادة بناء نظام التسويق داخل المنصة باستخدام React وTypeScript وPostgreSQL وAPI المنصة الموحدة.
- إضافة الحملات والأجندات وCreative Instances وTask Template والتاسكات التنفيذية والعلاقات الدقيقة بين الكاتب واليوزر التنفيذي.
- نقل إعدادات التسويق إلى صفحة الإعدادات الموحدة، مع الأقسام والكرييتيفات والإجراءات والمنصات وأنواع النشر والحالات وتصنيفات الباقات.
- ربط الاستوك وطلبات التصوير بنفس سجلات نظام العمليات، وإضافة التقويم والحضور والتقارير والباقات وتجهيز النشر وربط المنصات.
- ملف الترحيل المنظم: `database/migrations/20260723_marketing_native_rebuild.sql`.
- تعليمات التنفيذ والاختبارات داخل مجلد `docs/`.
