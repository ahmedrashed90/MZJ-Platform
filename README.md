...
## Operations Native V2 (v1.14.0)

The unified platform now includes a native Operations module backed by PostgreSQL, centralized sessions/permissions, atomic movements and transfer requests, delivery approvals, tracking linkage, safe deletion/audit flows, and dashboard drill-downs.

Before deploying, read:

- `docs/OPERATIONS-NATIVE-V2-IMPLEMENTATION-REPORT-AR.md`
- `docs/OPERATIONS-NATIVE-V2-DEPLOYMENT-AR.md`
- `database/migrations/20260720_operations_native_v2.sql`

## v1.15.5 - CRM completed workflow, unclassified messages and contact files

- توحيد الحالة النهائية لمبيعات التمويل باسم «تم البيع» مع ترحيل المسميات القديمة وتحديث معادلات البيع وجودة المبيعات.
- إخفاء عملاء «تم البيع» في الكاش والتمويل، وعملاء «تم الانتهاء» في خدمة العملاء، من داش بورد المتابعة فقط مع بقائهم في قاعدة البيانات والتقارير.
- تحويل صفحة «صندوق الوارد الموحد» إلى صفحة «رسائل غير مصنفة» مخصصة للرسائل التي لم يتم تحديد خدمتها بعد، مع واجهة احترافية وتصنيف إداري مباشر.
- إضافة صفحة «جهات الاتصال» بملف موحد للعميل يضم البيانات والحقول الإضافية والملاحظات والطلبات والمحادثات والرسائل وسجل الحالات والتوزيع.
...
## v1.15.8 - CRM layout refinement and voice recording

- إعادة تنسيق صفحة تقييم المناديب KPI بالكامل مع نافذة تقييم شاشة كاملة وألوان النسب الثلاثية المعتمدة من النظام القديم.
- تحسين تنسيق فلاتر التقارير وإبراز أسماء المناديب والعملاء.
- تبسيط رأس وفلاتر سجل عملاء التمويل بدون البلوك الملون السابق.
- حذف صفحة سجل ملكية العملاء ومسارها المخصص، مع الاحتفاظ بسجل تغييرات المسؤول داخل ملف جهة الاتصال.
- إضافة تسجيل فويس من داخل محادثة العميل وتحويل التسجيل إلى مرفق صوتي يمر بنفس مسار التخزين والإرسال الحالي.

## v1.15.9 - CRM reports, KPI PDF correction and readable typography
- إزالة تسجيل الفويس من واجهة محادثة العميل بالكامل مع الإبقاء على منطق المرفقات الحالي دون تغيير.
- تكبير الخط العام ورفع الوزن إلى Bold لتحسين القراءة داخل المنصة، مع تحسين إضافي لجداول CRM ونموذج KPI.
- إضافة إجمالي المصادر وإجمالي المبيعات في مصادر التسويق الرقمي والمباشر، وإجمالي الأقسام والفروع والمبيعات، وإجمالي المناديب والمبيعات.
- إجمالي المبيعات في التقارير يعتمد حصريًا على عمود «تم البيع» لكل صف ضمن نفس الفلاتر الحالية.
- إضافة تصدير PDF داخل نافذة «تقارير العملاء» يشمل كل النتائج المطابقة للبحث والفلاتر وليس الصفحة الظاهرة فقط.
- توسيط أزرار PDF في نموذج تقييم المناديب.
- فصل محتوى PDF السرعة والكفاءة والانضباط والقيمة والنتيجة، بينما PDF كامل يجمع كل الأقسام.
- لا توجد تغييرات في قاعدة البيانات أو API أو التوزيع أو الصلاحيات أو أي نظام خارج CRM.

## v1.16.2 - Operations workflow and inventory corrections

- فتح تفاصيل مخزون الإدارات ونواقص السيارات من الداش بورد داخل نافذة كاملة الشاشة.
- إعادة تنسيق عرض التشييك داخل ملف السيارة ونموذج حركة سيارات الوكالة بدون تداخل بين الاسم والحالة والملاحظة.
- تصغير زر طلب التراكينج داخل جدول المخزون وتحويله إلى مؤشر تقدم يتغير لونه حسب النسبة.
- ترقية جدول `operations.vehicle_check_history` تلقائيًا لإضافة الحقول الناقصة، ومنها `note`، قبل تنفيذ حركات السيارات.
- السماح بطلب حالة «مباع تم التسليم» مباشرة: التنفيذ فوري عند اكتمال الموافقتين، أو يتحول إلى طلب موافقات ثم يُنفذ تلقائيًا بعد اكتمالهما.
- مسح السيارة نهائيًا من مخزون السيارات بعد تأكيد رقم الهيكل، مع إزالة البيانات التشغيلية المرتبطة داخل معاملة واحدة والاحتفاظ بسجل تدقيق عملية المسح.

## v1.16.3 — Operations FK and complete vehicle delete

- توحيد المفتاح الخارجي لسجل التشييك مع `operations.check_item_definitions` مع الحفاظ على الأكواد القديمة، لمنع فشل حركة السيارات بسبب `safety_bag`.
- الإبقاء على زر مسح السيارة داخل مخزون السيارات، وتنفيذ الحذف داخل معاملة واحدة مع إزالة بيانات التراكينج والرسائل المرتبطة.

## v1.16.6 — ERPNext email-only user link

- ربط مندوب NEXT ERP بالمنصة يعتمد على الإيميل المسجل في حقل `ID المستخدم في NEXT ERP` فقط.
- تم حذف حقل وشرط مطابقة فرع NEXT ERP من نموذج المستخدم وAPI الربط.
...
## v1.17.0 — Automatic vehicle archive

- إصلاح قفل سجل السيارة أثناء الأرشفة باستخدام `FOR UPDATE OF v` بدل قفل الطرف القابل لأن يكون فارغًا في `LEFT JOIN`.
- توحيد قرار أرشفة السيارة داخل مسار واحد يعتمد فقط على اكتمال الموافقة المالية والإدارية، وعدم وجود طلب نقل جاري أو غير مكتمل، ووصول مراحل التراكينج المرتبطة بالسيارة إلى 100%.
- تشغيل فحص الأرشفة تلقائيًا عند اكتمال مرحلة التراكينج، وعند تحديث الموافقات، وعند حذف أو إلغاء أو إنهاء طلب النقل.
- الحفاظ على نفس واجهات وتصميم العمليات والتراكينج، وعدم إضافة جدول أو Migration أو مسار Webhook جديد.
