...
## v1.14.0 — نظام العمليات Native داخل المنصة الموحدة

- إعادة بناء نظام العمليات داخل React/PostgreSQL باستخدام تسجيل الدخول والقائمة الجانبية والصلاحيات والتصميم الموحد للمنصة.
- إضافة صفحات المخزون، إدارة السيارات، حركة السيارات، طلبات النقل والتصوير، تقرير جميع السيارات، وسجل الحركات.
- عدم إنشاء داش بورد عمليات مستقلة؛ مؤشرات العمليات تستمر داخل الداش بورد الموحدة من نفس جداول PostgreSQL.
- استبعاد صفحة التراكينج وصفحة Media القديمتين من موديول العمليات بالكامل، مع بقاء نظام التراكينج المستقل وMedia الخاصة بـCRM بدون تغيير.
- إضافة مراحل طلبات النقل والتصوير الأربع مع سجل منفذ وتوقيت كل مرحلة، وتنفيذ استلام سيارة النقل داخل Transaction واحدة.
- إضافة الاعتماد المالي والإداري، النواقص، الحركات المباشرة الجماعية، الأرشفة، والاستيراد والتصدير بصيغة CSV.
- الأرشفة تتحقق من حالة التسليم والاعتمادين ووجود حركة واكتمال طلب التراكينج المرتبط برقم الهيكل، بدون نسخ أي صفحة أو API من نظام التراكينج داخل العمليات.
- إضافة صلاحيات عمليات تفصيلية وتطبيقها داخل الـAPI مع تقييد البيانات حسب فروع المستخدم.
- إضافة إعدادات المواقع وحالات السيارات داخل صفحة الإعدادات الموحدة.
- جميع النوافذ والأدراج الجديدة تستخدم آلية المنصة المركزية للإغلاق بزر `Esc`.
- Migration الترقية موجود في `database/migrations/20260719_operations_native.sql`.
