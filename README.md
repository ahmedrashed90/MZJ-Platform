...
- إضافة وضع آمن للانتقال: **إلزام الحضور متوقف افتراضيًا** حتى ينتهي مدير النظام من ضبط جداول اليوزرات.
- أثناء الوضع الآمن كل اليوزرات يقدروا يدخلوا المنصة عادي، حتى لو عندهم جدول قديم أو غير مكتمل، ولا يعمل الـCron تسجيل انصراف أو Logout تلقائي بسبب الحضور.
- بعد تجهيز الجداول، مدير النظام يفعّل **تطبيق الحضور على تسجيل الدخول** من إعدادات الحضور والانصراف؛ عند التفعيل تُنهى جلسات اليوزرات المعيّن لهم جداول ليبدأوا دورة حضور جديدة صحيحة.
- تعديل الجداول أثناء الوضع الآمن لا يطرد اليوزرات الموجودين داخل المنصة.
- إصلاح سبب فشل الـDeployment الظاهر في V76: إعادة baseline إصدار الحزمة إلى `1.19.16` حتى ينجح فحص Marketing Packages + MZJ Club بدل `15/16`.
- الحفاظ على كل تعديلات V76: الفترات البديلة مثل «متواصل»، تعديل تعيين كل يوزر، الفرع، المكان المطلوب، يوم الإجازة، والتقارير الديناميكية.
- النسخة كاملة ونظيفة بدون Patch على Patch.

راجع `DELIVERY-GLOBAL-ATTENDANCE-CLEAN-V77.md`.

---

# تحديث العمليات — ملاحظة بدون حركة

- يمكن حفظ «حجز - نواقص - تحديد مكان» لنفس السيارة حتى لو لم يتغير المكان أو الحالة.
- التحديث يظهر في سجل الحركات كـ `note_update` ولا يغير مكان السيارة أو حالتها.
- تم إصلاح توافق `event_outbox` مع مخططات قاعدة البيانات القديمة.

راجع `docs/OPERATIONS-MOVEMENT-NOTE-ONLY-OUTBOX-COMPAT-AR.md`.

---

# MZJ Platform — إعادة بناء ربط المنصات v1.21.2

## سيستم التسويق — ربط المنصات

- إعادة بناء التبويب الموجود بالكامل بدل نموذج التوكنات اليدوي.
- OAuth حقيقي لـMeta وTikTok وYouTube.
- PostgreSQL وتشفير التوكنات، مع فحص وتجديد وRevoke عند الفصل.
- اختيار Facebook Page وربط Instagram Professional بدون قطع الاتصال الحالي قبل اكتمال الاختيار.
- استخدام صلاحيات المنصة المركزية وسجل عمليات بدون كشف التوكنات للواجهة.
- Migration انتقال نظيف يجعل أي ربط يدوي قديم بحاجة لإعادة OAuth.

راجع `docs/MARKETING-PLATFORM-CONNECTIONS-CLEAN-REBUILD-V1212-AR.md`.

---

# MZJ Platform — تسليم v1.20.7

## تفاصيل التسويق وقاعدة البيانات

- تفاصيل Task Template بعرض الشاشة بالكامل وبدون الهوامش الكبيرة.
- شاشة «عرض البيانات» في قاعدة بيانات التسويق أصبحت Full Screen.
- تنظيم بيانات الحملة والتاسكات والملفات والميزانية والنتائج والروابط.
- جدول النشر مجمع حسب اليوم والمنصة وبأعمدة اليوم والمنصة ونوع النشر.
- بدون Migration أو تغيير API أو صلاحيات أو منطق الفلو.

راجع `docs/MARKETING-DETAILS-DATABASE-V1207-AR.md`.

---

# MZJ Platform — تسليم v1.20.6

## داش بورد التسويق — جاهزية المطلوب

- تصغير بلوكات قسم المحتوى وقسم المونتاج بدون تغيير ترتيبها أو الفلو.
- إزالة الملخص الدائري الكبير واستبداله بملخص صغير لعدد التاسكات ونسبة القسم.
- إظهار عدد تاسكات الحملة ونسبة اكتمالها بشكل واضح خارج شريط التقدم.
- تفتيح شريط التقدم مع الحفاظ على ألوان المسؤولين من إعدادات التسويق.
- الحفاظ على التحديث اللايف وكل إجراءات التكليف والاستلام كما هي.
- لا توجد Migration أو تغييرات API أو قاعدة بيانات.

راجع `docs/MARKETING-DASHBOARD-COMPACT-V1206-AR.md`.

---

# MZJ Platform — تسليم v1.20.4

## داش بورد التسويق

- إخفاء تكرار كاتب المحتوى عندما يكون هو نفس المسؤول.
- استخدام ألوان المسؤولين المحفوظة في إعدادات سيستم التسويق بدل الألوان الثابتة.
- عرض نسبة الاكتمال بأرقام واتجاه واضحين داخل نفس التصميم.
- تحديث تلقائي لايف للداش بورد عند إجراءات التكليف مع الحفاظ على الكروت المفتوحة.
- بدون Migration أو تغيير للفلو أو الصلاحيات أو الأنظمة الأخرى.

راجع `docs/MARKETING-DASHBOARD-LIVE-COLORS-V1204-AR.md`.

---

# MZJ Platform — تسليم v1.20.2

## مراجعة واعتماد Task Template

- تحديد الحقول المطلوبة للتعديل باللون الأصفر.
- ملاحظة مستقلة داخل كل حقل بدل نافذة المتصفح.
- ظهور الملاحظات للمستخدم المكلّف عند طلب التعديل.
- أزرار مراجعة واعتماد احترافية وثابتة.
- تنظيم `TASK - المطلوب` و`جاهزية المطلوب` حسب الحملة والقسم.
- الحفاظ على انتقال التاسك الحالي عند «تم الاستلام».
- بدون Migration أو تغيير للأدوار أو منطق الحالات.
...
## تسليم إصلاح v1.20.1 — Task Template

- إصلاح رفض رفع وإعادة رفع Task Template للمستخدم صاحب الصلاحية والتكليف الفعلي.
- تحويل شاشة التفاصيل إلى Full Screen وتنظيم النصوص الطويلة، خصوصًا السكريبت الأساسي.
- تحويل إجراءات التكليف من علامات اختيار إلى أزرار حالات.
- تنزيل ملف `.xlsx` حقيقي بنفس نموذج الأعمدة المرجعي، بدون تحذير اختلاف الامتداد.
- لا توجد Migration، ولا تعديل على الأدوار أو `role_id` أو أقسام المستخدمين.

راجع `docs/MARKETING-TASK-TEMPLATE-V1201-AR.md`.

## تسليم إصلاح v1.20.0 — يوزرات قسم المحتوى

- إصلاح اختلاف قراءة يوزرات قسم المحتوى بين إعدادات التسويق وإنشاء الحملة والأجندة.
- الـAPI يرجع `contentDepartmentId` الصريح ويثبت علامة المحتوى على UUID الصحيح.
- نفس المستخدم يظل ظاهرًا في كل أقسام التسويق المسموحة له، مثل المحتوى والمونتاج.
- لا يتم حذف الأقسام أو العضويات أو تغيير القسم الأساسي أو `role_id`.

راجع `docs/MARKETING-CONTENT-DEPARTMENT-USERS-V1200-AR.md`.
...
## v1.19.4 — ربط إلغاء NEXT ERP ومزامنة التتبع

- مزامنة المرحلة الواحدة على جميع أرقام الهيكل داخل نفس طلب التتبع.
- حفظ حالة زر SMS+ باللون الأخضر بعد نجاح الإرسال.
- استقبال `Sales Order on_cancel` على نفس Endpoint مع تمييز نسخة الطلب برقم الطلب وتاريخ إنشاء NEXT ERP.
- إلغاء التراكينج وربط العمليات ودورة الموافقات وتسجيل CRM بدون حذف التاريخ.
- الاحتفاظ بالطلبات الملغية داخل تبويب طلب البيع في بيانات السيارة.
- لم يتم تغيير كتالوج الصلاحيات أو تصميم الصفحات.

## v1.19.3 — إصلاح SQL المركزي النظيف

- إزالة الفاصلة الزائدة من آخر صف في `core.system_pages` قبل `ON CONFLICT`.
- تصحيح المصدر نفسه في الـMigration والـSeed وBootstrap السيرفر، وليس إنشاء Patch جانبي.
- إضافة فحص رجوع يمنع أي فاصلة زائدة قبل `ON CONFLICT` في المصادر الثلاثة.
- لا يوجد أي تغيير في الفلو أو الصلاحيات أو قاعدة البيانات الوظيفية.

راجع `docs/V1193-CENTRAL-SQL-SYNTAX-FIX-AR.md`.

## v1.19.2 — إصلاح ترقية قاعدة الصلاحيات وتسجيل الدخول

- ضمان تطبيق مخطط الصلاحيات المركزي قبل المصادقة على قواعد البيانات الموجودة مسبقًا.
- تشغيل الترقية Idempotent تحت PostgreSQL advisory lock، مرة واحدة عند الحاجة.
- الاحتفاظ بإصلاح أنواع PostgreSQL وتوافق NodeNext من v1.19.1.
- بدون تغيير أي فلو تشغيلي.

راجع `docs/V1191-BUILD-TYPE-FIX-AR.md`.

## v1.19.0 — نظام المستخدمين والصلاحيات المركزي

- صفحة **الإعدادات ← المستخدمون والصلاحيات** هي المصدر الوحيد لإدارة المستخدمين والأدوار والأنظمة والفروع والأقسام ونطاق البيانات والصلاحيات الفردية والسجلات.
- تبويبات مستقلة للعمليات والتراكينج والتسويق وCRM لكل مستخدم، مع تفعيل النظام ودور النظام والفروع والأقسام والفرع والقسم الأساسيين.
- حماية الواجهة والراوت والـAPI والاستعلامات ومراحل سير العمل، مع تطبيق نطاق البيانات داخل السيرفر.
- تعطيل النظام يحتفظ بإعداداته المخزنة لكنه يمنع صفحاته وواجهاته وبياناته حتى إعادة التفعيل.
- 215 مفتاح صلاحية، و37 صفحة، و12 نطاق بيانات، مع Migration وSeed وRollback وتقارير مراجعة واختبارات.
- لم تتغير حالات الأنظمة أو مراحلها أو توزيع العملاء والتاسكات أو الحسابات التشغيلية.
...
تم تصغير كروت جاهزية المطلوب والتاسكات وتفتيح أشرطة التقدم وتوضيح نسبة الاكتمال، عبر تعديل CSS الأصلي مباشرة دون تغيير الفلو أو التحديث اللايف أو ألوان المسؤولين.
